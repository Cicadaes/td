  ...
  ngOnInit() {
    ...
    this.router.events.subscribe(event => {
      if (event instanceof NavigationEnd) {
        if (!(event.url == '/' || event.url.indexOf('product-center') >= 0)) {
          _that.routerList2 = localStorage.getItem('navList') && JSON.parse(localStorage.getItem('navList'));
        } else {
          _that.routerList2 = this.commonService.navList;
        }
      }
    });
  }
