...
export class CapacityPathDetailComponent extends BaseComponent {
  ...
  _isShowFunnelAddDialog: boolean;
  _eventList: any[] = [];
  _funnelParam: any = {};
  totalPath: any = 0;

  constructor(private injector: Injector, private service: CapacityPathService) {
    super(injector);
  ...
  _getEventList(list: any) {
    const eventList = [];
    if (list && list.length > 0) {
      for (let i = 0; i < list.length; i++) {
        eventList.push({
          value: list[i].pointid || '',
          label: list[i].pointname || ''
        });
      }
    }
    return eventList;
  ...
  showFunnelAddDialog(data?: any) {
    if (data && data.resultDetail) {
      this._eventList = this._getEventList(data.resultDetail);
    }
    /*if (this._eventList.length < 2 || this._eventList.length > 8) {
            this.message.create('warning', '转化漏斗必须最少2个步骤并且最多8个步骤。');
            return;
        }*/
    this._isShowFunnelAddDialog = true;
  }

  hideFunnelAddDialog(value: boolean) {
    this._isShowFunnelAddDialog = false;
  }

  onSubmitAddFunnel(value: boolean) {}
  ...
  queryById() {
    ...
    this.service.queryById(params).subscribe((response: any) => {
      if (response && response.list) {
        ...
        this._funnelParam = this._capacityPathDetails;
      }
    });
  }
