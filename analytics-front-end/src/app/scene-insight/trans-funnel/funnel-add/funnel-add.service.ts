...
export class FunnelAddService extends CurdService {
  ...
  constructor(private injector: Injector) {
    super(injector);
    this.queryEventUrl = this.reportBaseUrl + '/behaviorAnalysis/queryDictionaryList?productId=';
  }
  ...
  getStepSelect_eventName($param: any) {
    const param = {
      dicItemKey: 'eventid',
      eventTypeId: $param.eventTypeId,
      productId: $param.productId,
      sdkId: $param.sdkId
    };
    const configUrl: any = `${this.reportBaseUrl}/behaviorAnalysis/queryDictionaryList?productId=${$param.productId}`;
  ...
  setStepValue($param: any) {
    ...
    a['productId'] = this.getProductId();
    a['name'] = $param.nameValue;
    a['sdkId'] = $param.sdkId || null;
    const steps = [];
    ...
    return this.http.post(configUrl, a).pipe(catchError(this.handleError));
  }
