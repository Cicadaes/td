...
export class FunnelSeeService extends CurdService {
  ...
  getQueryEventUrl() {
    const queryEventUrl = `${
      this.reportBaseUrl
    }/behaviorAnalysis/dictionarys/${this.getProductId()}?sort=create_time&order=desc&dicKey=`;
    return queryEventUrl;
  }
  ...
  getFilter() {
    const configUrl: any = `${
      this.reportBaseUrl
    }/behaviorAnalysis/profilemetas/${this.getProductId()}?displayType=Tag&showCrowd=true`;
  ...
  getChartData($param: any) {
    ...
    return this.http.post(configUrl, p).pipe(catchError(this.handleError));
  }
      ...
      for (let i = 0; i < propSelect.length; i++) {
        ...
        if (!eventFilter && values && values.length > 0) {
          for (let j = 0; j < values.length; j++) {
            if ((option.label === '事件属性' || option.label === '页面参数') && value === values[j].value) {
              eventFilter = true;
              break;
            }
          }
  ...
  getChartData_advanced($filter: any) {
    if ($filter.advanced) {
      ...
      if (
        $filter.propValue &&
        ($filter.propValue.groupKey === '事件属性' || $filter.propValue.groupKey === '页面参数')
      ) {
        eventFilter = true;
      }
      return [
