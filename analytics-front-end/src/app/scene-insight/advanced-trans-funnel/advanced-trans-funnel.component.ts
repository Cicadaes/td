...
@Component({
  selector: 'app-advanced-trans-funnel',
  templateUrl: './advanced-trans-funnel.component.html',
  styleUrls: ['./advanced-trans-funnel.component.less']
})
export class AdvancedTransFunnelComponent extends BaseComponent implements OnInit, OnChanges {
  tabList1: any = [
    {
      name: 'App转化漏斗',
      filter: 'App'
  ...
  ngOnInit() {
    ...
    this.isVisitor = JSON.parse(localStorage.getItem('IsVisitor')) || false;
    if (this.isVisitor) {
      let ids = [];
  ...
  setTabList(ids: any) {
    ...
    this.pagePathFilter = this.globals.getStorageLocal('advanced_trans_funnel_tab')
      ? this.globals.getStorageLocal('advanced_trans_funnel_tab')
  ...
  setTab(value: any, index: any) {
    this.globals.setStorageLocal('advanced_trans_funnel_tab', value);
