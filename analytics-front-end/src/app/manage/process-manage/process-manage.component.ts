...
export class ProcessManageComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  constructor(private injector: Injector, private processManageService: ProcessManageService) {
    super(injector);
  ...
  confirmHideDialog(value: any) {
    // 判断是上线通过，还是上线拒绝
    if (this._item.status === 2) {
      // 通过上线
      this.processManageService.approve(this._item.id).subscribe(
        (response: any) => {
          if (response.code === 200) {
            this.getProcessList(this.parmas);
            this.flag = false;
      ...
      this.processManageService.downLine(this._item.id).subscribe(
        (response: any) => {
          if (response.code === 200) {
            this.flag = false;
            this.getProcessList(this.parmas);
