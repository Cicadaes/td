...
export class UserConfiguredService extends CurdService {
  ...
  getCheckedArrtibute(params: any) {
    ...
    return this.http
      .get('/crowd/config/metaAttributeGroup/listDetails' + stringParams)
  ...
  getAttributeCount(params: any) {
    const stringParams = this.getParams(params);
    return this.http.get('/crowd/admin/param/query' + stringParams).pipe(catchError(this.handleError));
  }

  // /crowd/config/metaAttributeGroup 保存属性接口
  insertArrtibute(params: any) {
    return this.http.post('/crowd/config/metaAttributeGroup', params).pipe(catchError(this.handleError));
  }

  // /crowd/config/configCrowdPortraits 查询人群画像配置列表
  getPortrait(params: any) {
    const stringParams = this.getParams(params);
    return this.http.get('/crowd/config/configCrowdPortraits' + stringParams).pipe(catchError(this.handleError));
  }

  // /crowd/config/configCrowdPortraits 新建人群画像组
  insertPortrait(params: any) {
    return this.http.post('/crowd/config/configCrowdPortraits', params).pipe(catchError(this.handleError));
  }

  // /crowd/config/configCrowdPortraits/{configCrowdPortraitId}  编辑人群画像组
  editPortrait(configCrowdPortraitId: any, params: any) {
    return this.http
      .put('/crowd/config/configCrowdPortraits/configCrowdPortrait', params)
      .pipe(catchError(this.handleError));
  }
  ...
  daletePortrait(configCrowdPortraitId: any, reportId: any, productId: any) {
    const url = `/crowd/config/configCrowdPortraits/${configCrowdPortraitId}/${reportId}/${this.getProductId()}`;
  ...
  checkPortraitName(params: any) {
    return this.http
      .post('/crowd/config/configCrowdPortraits/checkConfigCrowdPortraitName', params)
      .pipe(catchError(this.handleError));
  }
  ...
  checkPortraitSortOrder(params: any) {
    return this.http
      .post('/crowd/config/configCrowdPortraits/checkConfigCrowdPortraitSortOrder', params)
      .pipe(catchError(this.handleError));
  }
  ...
  queryTagCategories(params: any) {
    return this.http
      .post(this.crowdApiBaseUrl + '/tag/tagCategory/tree/query', params)
      .pipe(catchError(this.handleError));
  }

  /*创建标签分类*/
  createTagCategory(params: any) {
    return this.http.post(this.crowdApiBaseUrl + '/tag/tagCategory/create', params).pipe(catchError(this.handleError));
  }

  /*编辑标签分类*/
  editTagCategory(params: any, item: any) {
    const url = `${this.crowdApiBaseUrl}/tag/tagCategory/${item.id}/update`;
    return this.http.post(url, params).pipe(catchError(this.handleError));
  }

  /*删除标签分类*/
  delTagCategory(item: any) {
    const url = `${this.crowdApiBaseUrl}/tag/tagCategory/${item.id}/delete`;
