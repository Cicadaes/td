...
export class TagListTableComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  constructor(private service: UserConfiguredTagService, public globals: Globals, private injector: Injector) {
    super(injector);
  }

  currentPageDataChange(
    $event: Array<{ name: string; age: number; address: string; checked: boolean; disabled: boolean }>
  ): void {
  ...
  removeTags() {
    ...
    if (this._batchSelectedData && this._batchSelectedData.length === 1) {
      this.delete(this._batchSelectedData[0]);
      //          this._checkDeleteTagType = 'delete';
      //          this.deleteFunc(this._batchSelectedData[0]);
    } else {
  ...
  onSearchMoreSearch(params: any) {
    if (params.updateDataTimeRange) {
      params.updateDataTimeRange.end =
        this.globals.getDateZeroTime(params.updateDataTimeRange.end) + 24 * 3600 * 1000 - 1;
    }
    if (params.updateTagRuleTimeRange) {
      params.updateTagRuleTimeRange.end =
        this.globals.getDateZeroTime(params.updateTagRuleTimeRange.end) + 24 * 3600 * 1000 - 1;
    }
