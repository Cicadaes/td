...
export class UserConfiguredTagService extends CurdService {
  ...
  moveBatchTags(params: any, tagCate: any) {
    const url = `${this.crowdApiBaseUrl}/tag/customTags/p/${this.getProductId()}/c/${tagCate.id}/move`;
    return this.http.post(url, params);
  }
  ...
  checkCreateTags(params: any) {
    const url = `${this.crowdApiBaseUrl}/tag/customTags/p/${this.getProductId()}/create/check`;
    return this.http.post(url, params);
  }
}
