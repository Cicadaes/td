...
export class AppPushComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  constructor(private applyManageService: ApplyManageService, private injector: Injector) {
    super(injector);
  }
  ...
  confirmHideDialog(type: any) {
    ...
    this.applyManageService.deleteAndroidPush(this.appKey, this._item.channel).subscribe(
      response => {
        if (response.code === 200) {
          //                const json = {
