...
@Component({
  selector: 'app-app-push-manage',
  templateUrl: './app-push-manage.component.html',
  styleUrls: ['./app-push-manage.component.less']
})
export class AppPushManageComponent extends BaseComponent implements OnInit, OnChanges {
  appConf: any = {
    appConfig: {},
    iosChannelConfig: {}
  }; // app配置信息
  ...
  constructor(private appPushManageService: AppPushManageService, private injector: Injector) {
    super(injector);
    this.initRouterList('应用推送配置');
  }
  ...
  getAppConfig() {
    //        const that = this;
    this.appPushManageService.getAppConfig().subscribe(response => {
  ...
  initAndroidPushConfig(appData: any) {
    ...
    if (data && data[0]) {
      if (
        !(data[0].jpushKey || data[0].jpushSecret) &&
        !(data[0].getuiApp || data[0].getuiKey || data[0].getuiSecret)
      ) {
  ...
  bind(index: any) {
    ...
    (data['app'] = this.appKey), (data['pid'] = that.productId);
    ...
    this.appPushManageService.updateAndroidPush(data).subscribe(response => {
      if (response.code === 200) {
  ...
  delete(index: any) {
    const that = this;
    const data = that.pushConfigList[index];
    delete that.isError[index];
    this.appPushManageService.deleteAndroidPush(this.appKey, data.channel).subscribe(response => {
      if (response.code === 200) {
        //                const json = {
  ...
  addConf() {
    let channel: any;
    let value: any;
    if (this.pushConfigList[0] && this.pushConfigList[0].channel === 3) {
      // 当已有一条推送通道配置时 再添加 需要修改pushPlatform来让下拉框内容改变
      channel = 4;
      value = '极光推送';
      const arry = [];
  ...
  transDataToReq(channel: number, app: string, secret: string) {
    ...
    this.appPushManageService.updateAndroidPush(json).subscribe(response => {
      if (response.code === 200) {
        this.notification.create('success', '提示信息', '绑定成功');
      } else {
  ...
  upload(tp: number) {
    ...
    // 加提示file为空
    if (this.file) {
      this.appPushManageService
        .updateIOS(this.beforUpload(this.file, this.pwd), this.appKey, tp)
        .subscribe(response => {
          if (response.code === 200) {
            this.testTipShow = false;
            this.proTipShow = false;
