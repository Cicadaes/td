...
export class PageHeatmapTop10Component extends BaseComponent implements OnInit, OnChanges {
  ...
  downloadAllLinkData() {
    let title = '';
    const dateRange = `
      ${this.globals.dateFormat(this._queryParam.starttime_day[0], '')} 至${this.globals.dateFormat(
      this._queryParam.starttime_day[1],
      ''
    )}`;
    if (this._queryParam && this._queryParam.sourceid === 1) {
      title = '行为分析-页面热力图-Web页面-链接点击图-数据导出-' + dateRange;
    } else if (this._queryParam && this._queryParam.sourceid === 2) {
      title = '行为分析-页面热力图-H5页面-链接点击图-数据导出-' + dateRange;
    }
    const param = {
