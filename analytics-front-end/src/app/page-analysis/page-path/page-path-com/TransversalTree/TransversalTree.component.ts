...
export class TransversalTreeComponent implements OnInit, OnChanges {
  ...
  @Input()
  set selectId(selectId: any) {
    this.selectOption = selectId;
  }
  ...
  set rootList(rootList: any) {
    ...
    if (this._rootList && this._rootList.length > 0) {
      ...
      if (this._checkedValue && !this.isSearch) {
        let hasValue = false;
        for (let j = 0; j < this._rootList.length; j++) {
          if (this._rootList[j].id == this._checkedValue.id) {
            hasValue = true;
            break;
          }
        }
        if (!hasValue) {
          this._rootList.unshift(this._checkedValue);
        }
      }
  ...
  onSearch(value: any) {
    this.isSearch = value ? true : false;
