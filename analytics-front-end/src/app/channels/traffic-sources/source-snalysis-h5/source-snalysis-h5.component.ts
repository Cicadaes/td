...
import { trafficSourcesModel } from '../traffic-sources.model';
...
@Component({
  selector: 'app-source-snalysis-h5',
  templateUrl: './source-snalysis-h5.component.html',
  styleUrls: ['./../traffic-sources.component.less']
})
export class SourceSnalysish5Component extends BaseComponent implements OnInit {
  searchTerms = new Subject<string>();
  _tabsChange = new Subject<number>();
  sourcesnalysisModel: trafficSourcesModel = new trafficSourcesModel();
  ...
  popovers: any = this.sourcesnalysisModel.popovers; //渠道趋势tootip里面的内容
  tabsLists: any = this.sourcesnalysisModel.tabsLists; //Top10渠道趋势标签页数据
  ...
  details: any = this.sourcesnalysisModel.details; //来源明细tootip数据
  dataSet: any = []; //图表数据
  ...
  sort: any; //排序字段
  tableHeaders: any = this.sourcesnalysisModel.tableHeaders; //图表headerconter
  analysisObject: any; //更多筛选数据
  constructor(
  ...
  analyticTargetChange(data: any) {
    ...
    if (nub == 1) {
      this.sourcesnalysisModel.checkOptionsanalytic.forEach((cont: any, index: any) => {
        if (!cont.disabled && cont.type && cont.checked) {
          this.sourcesnalysisModel.checkOptionsanalytic[index].disabled = true;
        }
      });
    } else {
      this.sourcesnalysisModel.checkOptionsanalytic.forEach((cont: any, index: any) => {
        if (cont.disabled) {
          this.sourcesnalysisModel.checkOptionsanalytic[index].disabled = false;
        }
        if (!cont.type) {
          this.sourcesnalysisModel.checkOptionsanalytic[index].disabled = true;
        }
    ...
    this.checkOptionsanalytic = this.sourcesnalysisModel.checkOptionsanalytic;
    this.key = false;

    let _checkOptionsanalytic: any = this.deepCopy(this.checkOptionsanalytic);
    // let _checkOptionsanalytic: any = _.cloneDeeplodash(this.checkOptionsanalytic);
  ...
  analyticTarget(data: any) {
    for (let index = 0; index < data.length; index++) {
      if (!data[index].disabled) {
        this.sourcesnalysisModel.checkOptionsanalytic[index].checked = true;
        this.sourcesnalysisModel.checkOptionsanalytic[index].disabled = true;
        return;
      }
    }
      ...
      this.trafficSourcesService.checkCrowdName(parmes).subscribe((response: any) => {
        if (response && response.success && response.list && response.list.length) {
          for (let i = 0; i < response.list.length; i++) {
            this.sourcesnalysisModel.checkOptionsanalytic.forEach((element: any, index: any) => {
              if (`_td_${response.list[i].name}` == element.value) {
                switch (response.list[i].status) {
                  case 0:
                    this.sourcesnalysisModel.checkOptionsanalytic[index].disabled = false;
                    this.sourcesnalysisModel.checkOptionsanalytic[index].type = true;
                    break;
                  case 1:
                    this.sourcesnalysisModel.checkOptionsanalytic[index].disabled = true;
                    this.sourcesnalysisModel.checkOptionsanalytic[index].type = false;
                    break;
                  default:
                    break;
                }
        ...
        this.analyticTarget(this.sourcesnalysisModel.checkOptionsanalytic);
        this.checkOptionsanalytic = this.sourcesnalysisModel.checkOptionsanalytic;
        resolve();
      });
    });
