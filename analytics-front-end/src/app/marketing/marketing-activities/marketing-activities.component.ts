...
export class MarketingActivitiesComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  constructor(public marketingActivitiesService: MarketingActivitiesService, private injector: Injector) {
    super(injector);
  ...
  updatePipelineAll() {
    for (let i = 0; i < this.pipelineList.length; i++) {
      ...
      if (
        (this.actVO.status == 1 || this.actVO.status == 2 || this.actVO.status == 3) &&
