...
export class MarketingActivitiesService extends CurdService {
  ...
  createEquity(body: any) {
    const configUrl: any = `/marketing-api/campaign/equityConfigs?uploadUUID=${body.uploadUUID}&name=${
      body.name
    }&total=${body.total}&campaignId=${body.campaignId}`;
    return this.http.post(configUrl, {});
  }

  // 权益配置_更新
  updateEquity(body: any) {
    let configUrl: any = `/marketing-api/campaign/equityConfigs/${body.equityConfigId}?name=${body.name}`;
