...
export class ActivityCenterComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  dwUrlPrefix: string = 'https://aeplus.beta.data-whisper.com'; // dw 链接前缀

  dwUrl = {
    create: `/account/0/mass/message`,
    detail: `/account/mass/detail`,
    report: `/account/mass/articledetail`
  };
  ...
  constructor(
    ...
    window.addEventListener('message', (event: any) => {
      if (event.data && typeof event.data === 'string') {
        ...
        if (data && data.eventInfo && data.eventType === 'dwPre') {
          // dw窗口选择“上一步”
          $scope.modalRef.close();
          $scope.vm.oneDotData['name'] = data.eventInfo['name'];
          $scope.vm.oneDotData['description'] = data.eventInfo['description'];
          $scope.vm.oneDotData['channelType'] = 4; // 微信类型
          $scope.handlerScl(2);
        }
      }
    });
    ...
    // ========================
    const buttonList = localStorage.getItem('buttonList');
    if (buttonList) {
      for (let i = 0; i < buttonList.length; i++) {
        const obj = buttonList[i];
        if (obj['action'] === 'wechatgrouphair') {
          $scope.wechatSettingEnable = true;
        }
      }
  ...
  ngOnInit() {
    ...
    $scope.activityCenterService.getDwUrlPrefix().subscribe(response => {
      if (response.code === 200) {
        $scope.dwUrlPrefix = response.data;
      }
    });
    ...
    successByLaunchWaysRadio = function($data: any) {
      ...
      if ($data) {
        $data.forEach(element => {
          if (element.value != 4 || $scope.wechatSettingEnable) {
            arr.push(element);
          }
        });
      }
      $scope.vm.singleContactLaunch.launchWaysRadio = arr;
    };
  ...
  handlerScl(type: number, e?: any) {
    ...
    $scope.activityCenterService.getCrowdList(this.crowdId).subscribe((response: any) => {
      if (response.code === 200) {
        if (response.data) {
  ...
  handlerDwPage(type: any, dwSegmentId: number) {
    ...
    if (type === 'create') {
      const sclData = $scope.scl.vm;
      url = `${this.dwUrlPrefix}${this.dwUrl.create}?winId=${this.guid()}&name=${sclData.launchNameValue}&productId=${
        this.productId
      ...
      url = `${this.dwUrlPrefix}${
        this.dwUrl.detail
      }?winId=${this.guid()}&id=${dwSegmentId}&token=${localStorage.getItem('token')}`;
    } else if (type === 'report') {
      url = `${this.dwUrlPrefix}${
        this.dwUrl.report
      }?winId=${this.guid()}&id=${dwSegmentId}&token=${localStorage.getItem('token')}`;
  ...
  handlerSclOperation_detail(data: any, isUpdate: number) {
    ...
    that.activityCenterService.getSegmentList(data.id).subscribe(response => {
      ...
      that.crowdId = response.data[0].crowdId;
    ...
    if (type === 1) {
      ...
      for (let i = 0; i < length; i++) {
        ...
        if (pushData[i]['customParameters'] && pushData[i]['customParameters'].length > 0) {
          for (let j = pushData[i]['customParameters'].length - 1; j >= 0; j--) {
            ...
            const first = '${',
              last = '}';
            if (pushData[i]['message'].indexOf(`${first}${element}${last}`) === -1) {
      ...
      if (json['customParameters'] && json['customParameters'].length > 0) {
        for (let j = json['customParameters'].length - 1; j >= 0; j--) {
          ...
          const first = '${',
            last = '}';
          if (json['content'].indexOf(`${first}${element}${last}`) === -1) {
  ...
  saveSingle(type: number, isAutoSub?: boolean) {
    ...
    const requestObj = this.processReqData(
      this.segmentDataService.segmentInfo,
      this.segmentDataService.segmentInfo['channelType']
    );
