...
export class ActivityCenterAddMcpComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  constructor(private activityCenterService: ActivityCenterService, private injector: Injector) {
    super(injector);
    ...
    if (valid) {
      ...
      if ($scope._manyDotData.hasOwnProperty('id')) {
        ...
        return new Promise((resolve: any, reject: any) => {
          $scope.activityCenterService.insertManyDotClon($scope._manyDotData).subscribe(
            (response: any) => {
        ...
        return new Promise((resolve: any, reject: any) => {
          $scope.activityCenterService.insertManyDot(parmas).subscribe(
            (response: any) => {
              if (response.code === 200) {
