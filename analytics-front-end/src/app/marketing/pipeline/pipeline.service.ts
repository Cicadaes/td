...
export class PipelineService extends CurdService {
  ...
  saveDraft(pipelineData: any) {
    ...
    return this.http.post(url, pipelineData).pipe(
      map(this.handleBad),
      catchError(this.handleError)
    );
  }
   ...
   * 获取触发器组件 事件列表
   */
  getEventListForTimerEcho(funnelEventCodeList: any, data?: any) {
    let url = `${this.baseUrl}/config/eventConfigs/all/echo?status=0`;
    if (funnelEventCodeList && funnelEventCodeList.length > 0) {
      url += `&eventCode=${funnelEventCodeList}`;
    }
    if (data) {
      let paramUrl = this.getParams(data);
      url = `${url}&${paramUrl.substring(1)}`;
   ...
   * 获取过滤器 过滤条件 标签列表
   */
  getTargetListForFilter(crowdId?: any) {
    let url = `${this.baseUrl}/crowd/crowds?page=1&pageSize=99999999`;
    if (crowdId) {
      url += `&crowdIds=${crowdId}`;
    }
   ...
   * 获取分流器 维度数据（回显）
   */
  getShuntDimensionsListEcho(dimensionCodeList: any) {
    let url = `${this.baseUrl}/config/dimensionConfigs/all/echo?status=0`;
    if (dimensionCodeList && dimensionCodeList.length > 0) {
      url += `&dimensionCode=${dimensionCodeList}`;
    }
    // url += `&dimensionCode=${dimensionCodeList}`;
    return this.http.get(url).pipe(
      map(this.handleBad),
   ...
   * scopeType 0 查所有 1 查规则条件指标  2 查触发器指标 3 查计划目标
   */
  getTriggerTargetListEcho(targetCodeList: any, scopeType: number) {
    let url = `${this.baseUrl}/config/targetConfigs/scope/${scopeType}/echo?status=0`;
    if (targetCodeList && targetCodeList.length > 0) {
      url += `&targetCode=${targetCodeList}`;
    }
    return this.http.get(url).pipe(
      map(this.handleBad),
      catchError(this.handleError)
  ...
  check(pipeline: any) {
    ...
    return this.http.post(url, pipeline).pipe(
      map(this.handleBad),
      catchError(this.handleError)
    );
  }
  ...
  generateBehaviorPath(nodeId: number, diagram: string) {
    ...
    return this.http.post(url, diagram).pipe(
      map(this.handleBad),
      catchError(this.handleError)
    );
  }
  ...
  /**
   * 提前终止规则 pipeLine 第一个下拉框数据 (回显)
   */
  getTerminationRuleEcho(campaignId: any, pipelineId: any, targets: any) {
    let url = `${this.baseUrl}/campaign/pipelineDefinitions/config/${campaignId}/${pipelineId}/terminationRule`;
    if (targets) {
      url += `?eventCode=${targets}`;
    }
    return this.http.get(url).pipe(
      map(this.handleBad),
      catchError(this.handleError)
