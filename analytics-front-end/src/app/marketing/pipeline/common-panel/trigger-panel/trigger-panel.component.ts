...
export class TriggerPanelComponent implements OnInit {
  ...
  constructor(public pipelineService: PipelineService, public pcs: PipelineCommunicationService) {
    ...
    // 获取事件列表
    that.pipelineService.getEventListForTimerEcho(eventArr, { triggerFlag: 1 }).subscribe(
      (result: any) => {
  ...
  addEvent() {
    ...
    if (
      length &&
      (branch[branch.length - 1].code === 'undefined' || !branch[branch.length - 1].code) &&
      that.pcs.nodeData.targetType === 'event'
    ) {
  ...
  addTarget() {
    ...
    const length = branch.length;
    if (
      that.pcs.nodeData.otherTarget === 'undefined' ||
  ...
  addConditions() {
    ...
    if (
      length != 0 &&
      (!conds[length - 1].target ||
        (!conds[length - 1].number && (!conds[length - 1].first || !conds[length - 1].second)))
    ) {
