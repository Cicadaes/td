...
export class ShuntPanelComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // 初始化维度相关数据
    that.pipelineService.getShuntDimensionsListEcho(that.pcs.nodeData.dimensionCode).subscribe((result: any) => {
        ...
        if (that.pcs.nodeData.dimensionCode && that.dimensionsList.length) {
          ...
          that.pipelineService.getShuntDimensionsOptionList(dimensionId).subscribe((result: any) => {
            if (result.code === 200) {
              ...
              if (branchLength && that.pcs.nodeData.branchList[0].optionCode) {
                let str = JSON.stringify(that.dimensionsOptionsList);
                for (let i = that.pcs.nodeData.branchList.length - 1; i >= 0; i--) {
                  const element = that.pcs.nodeData.branchList[i];
                  if (str.indexOf(element.optionCode) === -1) {
                    that.dimensionsOptionsList.unshift({
                      label: element.name,
  ...
  updateDimensionsOptions(data: any, index: number) {
    ...
    if (data.checked) {
      ...
      that.defaultData['deleteType'] = '';
      that.defaultData['index'] = index;
      that.defaultData['move_index'] = 'not';
      that.pcs.lineChangeMission(that.defaultData);
      let flag;
      for (let i = 0; i < that.pcs.nodeData.branchList.length; i++) {
