...
export class PushPanelComponent implements OnInit, OnDestroy {
  ...
  editorChange(event: any) {
    ...
    let description = event.replace(/<.*?>/gi, '');
  ...
  editorInsertVariable(data: any) {
    ...
    if (
      that.pcs.nodeData.message &&
      data.key.length + 3 + that.pcs.nodeData.message.replace(/<.*?>/gi, '').length > 240
    ) {
      // 3 是 ${}的长度
  ...
  insertVariable(data: any) {
    const that = this;
    if (that.pcs.nodeData.message && data.key.length + 3 + that.pcs.nodeData.message.length > 240) {
      // 3 是 ${}的长度
      this.notification.create('error', '错误提示', '剩余内容不足，无法插入变量');
      return;
    }
