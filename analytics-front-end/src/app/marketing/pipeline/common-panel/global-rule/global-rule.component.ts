...
export class GlobalRuleComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (that.pcs.globalRule.pipelineForbiddenRuleDefinition.ruleList) {
      that.pcs.globalRule.pipelineForbiddenRuleDefinition.ruleList.forEach(element => {
        if ('object' === typeof element.ruleType) {
          arr.push(element.ruleType.code);
        }
      });
    }
    that.handleData();
  ...
  async getTarget(): Promise<any> {
    ...
    let arr = [];
    if (that.terminationRuleList) {
      that.terminationRuleList.forEach(element => {
        arr.push(element.target.code);
      });
    }
    this.pipelineService
      .getTerminationRuleEcho(this.pcs.campaignId, this.pcs.pipelineId, arr)
      .subscribe((data: any) => {
        if (data.code === 200) {
          ...
          if (this.terminationRuleList.length > 0) {
            for (let a = 0; a < this.terminationRuleList.length; a++) {
              for (let b = 0; b < this.targetList.length; b++) {
                if (
                  this.terminationRuleList[a].target.code === this.targetList[b].value.code ||
  ...
  selectRule() {
    ...
    if (
      that.pcs.globalRule.pipelineEnterRuleDefinition.lessThanDays ||
      that.pcs.globalRule.pipelineEnterRuleDefinition.lessThanTimes
    ) {
      that.pcs.globalRule.pipelineEnterRuleDefinition.unlimited = false;
    } else {
      that.pcs.globalRule.pipelineEnterRuleDefinition.unlimited = true;
  ...
  addRule() {
    if (this.terminationRuleList.length > 0) {
      for (let i = 0; i < this.terminationRuleList.length; i++) {
        if (
          !(
            this.terminationRuleList[i].code &&
            this.terminationRuleList[i].number &&
            this.terminationRuleList[i].target
          )
        ) {
