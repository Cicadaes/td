    ...
    this.reportService.getSmsOverview(this.reportObj.segmentId).subscribe(response => {
      if (response.code === 200) {
        ...
        if (response.data && response.data[this.reportObj.segmentId] && data['sum_td_sms_send_count']) {
          // 成功率
          successRate =
            ((data['sum_td_sms_send_success_count'] / data['sum_td_sms_send_count']) * 100).toFixed(0) + '%';
          // 失败率
          ...
          // 无法连接
          notAvailabel =
            ((data['sum_td_sms_send_not_available'] / data['sum_td_sms_send_count']) * 100).toFixed(1) + '%';
          // 其他
