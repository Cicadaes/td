...
export class ReportEmailComponent implements OnInit, OnChanges {
    ...
    this.reportService.getEdmOverview(this.reportObj.segmentId).subscribe(response => {
      if (response.code === 200) {
        ...
        if (response.data && response.data[this.reportObj.segmentId] && data['sum_td_edm_send_count']) {
          ...
          // 无效地址
          nvalidAddressRate =
            ((data['sum_td_edm_send_invalid_address'] / data['sum_td_edm_send_count']) * 100).toFixed(1) + '%';
          // 账户关闭
          closedAccountRate =
            ((data['sum_td_edm_send_closed_account'] / data['sum_td_edm_send_count']) * 100).toFixed(1) + '%';
          // 账户余额不足
          sufficienRate =
            ((data['sum_td_edm_send_not_sufficient_funds'] / data['sum_td_edm_send_count']) * 100).toFixed(1) + '%';
  ...
  getCycleTimeList(type: string) {
    ...
    if (type === 'left') {
      ...
      if (length - index < 8) {
        // length为数组实际长度 index从0开始计算 所以要大于8
