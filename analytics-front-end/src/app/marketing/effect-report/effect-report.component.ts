...
export class EffectReportComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  getReportTemplateList() {
    ...
    that.effectReportService.getTemplates(params).subscribe(
      data => {
      ...
      if (data.code === 200) {
        ...
        list.forEach(element => {
          element['src'] = that.domSanitizer.bypassSecurityTrustResourceUrl(
            `${that.reportDefaultUrl}/studio/#/publish/${element.formId}?campaignId=${that.campaignId}&tenantId=${
              that.campaignInfo['tenantId']
            }&startDate=${that.campaignInfo['startTime'].substring(0, 10)}&endDate=${that.campaignInfo[
              'endTime'
            ].substring(0, 10)}&product_id=${that.productId}&token=${localStorage.getItem('token')}`
          );
  ...
  selectReport() {
    ...
    that.effectReportService.saveSelectReport(json).subscribe((data: any) => {
      ...
      setTimeout(() => {
        ...
        that.src = `${that.reportDefaultUrl}/studio/#/publish/${data.formId}?campaignId=${that.campaignId}`;
  ...
  setIframeHeight() {
    ...
    document.getElementById('tdReportIframe') &&
      (document.getElementById('tdReportIframe').style.height = `${tempHeight}px`);
  }
