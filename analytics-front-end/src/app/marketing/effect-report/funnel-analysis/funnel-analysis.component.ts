...
export class FunnelAnalysisComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  getFunnelView() {
    ...
    that.funnelAnalysisService.getFunnelConvertOverview(param).subscribe(resp => {
      ...
      that.funnelChart = cloneDeep(that.funnelChartOption);
  ...
  trendLineCharts() {
    ...
    that.funnelAnalysisService.getTrendChart(param).subscribe(data => {
      ...
      that.stepChart = cloneDeep(that.stepChartOption);
    });
  }
  ...
  lastEventTrendCharts() {
    ...
    that.funnelAnalysisService
      .getLastEventChart(
        that.tempFunnelData[0].funnelConvertOverviewEventStepItem[
          that.tempFunnelData[0].funnelConvertOverviewEventStepItem.length - 1
        ].eventId,
        param
      )
      .subscribe(data => {
        ...
        that.lastChart = cloneDeep(that.lastChartOption);
      });
  }
  ...
  formatFunnelViewData(data: any) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (that.selectedGenerateCrowd.length === 0) {
        ...
        view['wholeConvertRate'] =
          +(isNaN(+viewData[viewData.length - 1].val / +viewData[0].val) || !viewData[0].val
            ? 0
            : (+viewData[viewData.length - 1].val / +viewData[0].val) * 100
          ).toFixed(2) + '%';
  ...
  formatFunnelData(crowdName: any, data: any) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      that.steps.push({
        label: data[i]['stepOrder'],
        value: data[i]['stepOrder']
      });
      if (i > 0) {
        const json = {
  ...
  stepChartOption = {
    ...
    tooltip: {
      ...
      // 提示框配置
      formatter: function(params: any) {
        let res = `<div style="border-bottom:1px solid #dfe7f2;padding-bottom:12px;">
        时间<span style="margin-left:70px;">${params[0].name}</span>
        </div>`;
        for (let i = 0; i < params.length; i++) {
          res += `
          ...
          <span style="color:${params[i].color};margin-right:8px;">●</span>${
            params[i].seriesName
          }<span style="color: #2D8CF0;margin-left:90px;">${params[i].value}%</span>
          </p>`;
        }
        return res;
      }
  ...
  lastChartOption = {
    ...
    tooltip: {
      ...
      formatter: function(params: any) {
        let res = `<div style="border-bottom:1px solid #dfe7f2;padding-bottom:12px;">
        时间<span style="margin-left:70px;">${params[0].name}</span></div>`;
        for (let i = 0; i < params.length; i++) {
          res += `<p style="margin-top:12px;"><span style="color:${params[i].color};margin-right:8px;">●</span>${
            params[i].seriesName
          }<span style="color: #2D8CF0;margin-left:90px;">${Number(params[i].value).toLocaleString()}</span></p>`;
        }
        return res;
      }
    },
