...
@Injectable()
export class EventAnalysisDetailService extends CurdService {
  constructor(private injector: Injector) {
    super(injector);
  }

  // 查询属性备选列表
  metaAttributeList(eventId: any) {
    const param = `eventId=${eventId}&productId=${this.getProductId()}`;
    const configUrl = `${this.reportBaseUrl}/eventContrastAnalysis/profilemetas/${this.getProductId()}?${param}`;
    return this.http.get(configUrl);
  }

  // 查询行为属性备选列表
  behaviorAttributeList() {
