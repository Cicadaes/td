...
export class UserRetentionTableComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  constructor(private service: UserRetentionService, private injector: Injector) {
    super(injector);
    this.urlParams = {};
  }
  ...
  downloadData() {
    ...
    const startDate = this.globals.dateFormat(this._filter.dateRange[0], '');
    const endDate = this.globals.dateFormat(this._filter.dateRange[1], '');
  ...
  _getNumArea(response) {
    ...
    if (response) {
      const maxNum = response.maxNum || 0;
      const minNum = response.minNum || 0;
      const average = (parseFloat(maxNum) - parseFloat(minNum)) / 5;
      area = {
        one: average * 5,
        two: average * 4,
        three: average * 3,
  ...
  _getNewOperator(filter: any) {
    const operator = filter.operator;
    const displayType = filter.displayType;
  ...
  _buildQueryParam() {
    ...
    const filters = this._filter.filters;
    let moreFilter = {
      filter: '',
      operator: '',
      values: []
    ...
    if (filters && filters.length > 0) {
      ...
      const values = firstFilter.values;
      moreFilter.filter = firstFilter.filter;
      moreFilter.operator = this._getNewOperator(firstFilter);
      moreFilter.values = values;
    }
    this._queryParam = {
      total: false,
      sumMetric: false,
    ...
    if (moreFilter.values.length > 0) {
      const moreFilterJson = {
        type: 'and',
        condition: [
          {
            field: moreFilter.filter,
            operator: moreFilter.operator,
            value: moreFilter.values
          }
        ]
      };
      this._queryParam.filters.push(moreFilterJson);
    }
  }
