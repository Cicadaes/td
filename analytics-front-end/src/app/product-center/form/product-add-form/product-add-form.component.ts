...
export class ProductAddFormComponent implements OnInit, OnChanges {
  ...
  productnameAsyncValidator = (control: FormControl) =>
  ...
  initFormData() {
    ...
    if (this._data) {
      ...
      if (!this._data['miniprogramAppid'] && !this._data['miniprogramSecret']) {
        this._resetMiniprogrameValue();
      }
    } else {
      this._resetMiniprogrameValue();
    }
  ...
  _resetMiniprogrameValue() {
    setTimeout(() => {
      this.componentChange('', 'miniprogramAppid');
      this.componentChange('', 'miniprogramSecret');
      this._miniprogramAppidError.isError = false;
      this._miniprogramSecretError.isError = false;
    }, 100);
  }
