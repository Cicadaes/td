...
export class UserListComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  constructor(private userInsightService: UserInsightService, private injector: Injector) {
    super(injector);
  ...
  ngOnInit() {
    ...
    this.searchUserListData = JSON.parse(localStorage.getItem('userList')) || [];
    if (
      this.searchUserListData.length > 0 &&
      (this.searchUserListData[0].value || this.searchUserListData[0].value === 0)
    ) {
      this.searchFlag = true;
      this.boolFilters = this.searchUserListData;
    }
    ...
    this.searchFlag ? this.queryDeatil() : this.getTableData(this.parmas);
  }
  ...
  initParam() {
    // if (this.boolFilters && this.boolFilters.length < 1) {
    ...
    if (this.userInsightService.userListParmas && Object.keys(this.userInsightService.userListParmas).length != 0) {
      const obj = JSON.parse(JSON.stringify(this.userInsightService.userListParmas));
      if (obj && obj.hasOwnProperty('definition')) {
        this.parmas['definition'] = obj.definition;
      }
  ...
  getThead() {
    this.userInsightService.getThead().subscribe((response: any) => {
      ...
      if (response.data.length < 1) {
        this.tableNoData =
          '暂未对用户列表进行配置，请在管理-用户配置-用户洞察配置中进行配置，如果没权限，请联系管理员。';
      }
    });
  }
  ...
  getTableData(parmas: any) {
    this.userInsightService.getTableData(parmas).subscribe(
      (response: any) => {
  ...
  queryDeatil() {
    ...
    for (let k = 0; k < this.boolFilters.length; k++) {
      ...
      if (boolFilter.displayType === 'Tag' && boolFilter.value && boolFilter.value.length === 0) {
    ...
    if (failure) {
      ...
      // 装入当前条件对象
      const ont = {};
      if (this.parmas.definition.condition.condition01.queryList.length > 0) {
        ont['operator'] = 'and';
      }
  ...
  attributeChange(value: any) {
    ...
    if (value.length < 1) {
      this.userListDataTableLoading = true;
      this.userInsightService.userInsight['userList'] = [];
      this.userInsightService.userListParmas = this.userInsightService.userInsightParmas;
      this.initParam();
      this.parmas.page = 1;
      this._current = 1;
      this.getTableData(this.parmas);
  ...
  PageChange(e?: any) {
    ...
    if (e) {
      ...
      key = true;
      this.parmas.page = this._current;
    }
  ...
  ngOnDestroy() {
    ...
    switch (hash) {
      ...
      case '/user/user-profile':
        break;
      default:
        this.userInsightService.userInsight.attribute['name'] = '';
        break;
    }
  }
}
