...
export class BehaviorTrajectoryComponent extends BaseComponent implements OnInit {
  ...
  constructor(protected commonService: CommonService, private injector: Injector) {
    super(injector);
    $scope = this;
  }
  ...
  handlerChartOption() {
    ...
    if (b && b.length > 0) {
      ...
      // chartOption.series[0].label.formatter =
      // b.length > 0 ? '{text|' + b[0].value + '%}\n{subtext|' + name + '}' : '';
  ...
  handlerChartTooltip($data: any) {
    return `
                <style>
                    ...
                    .a5b1c668-fc10-45ea-b8ef-7ab30a6eddd9 .name,
                     .a5b1c668-fc10-45ea-b8ef-7ab30a6eddd9 .value,
                     .a5b1c668-fc10-45ea-b8ef-7ab30a6eddd9 .valueByCount {
                        padding: 8px;
                        font-size: 12px;
                        line-height: 12px;
                        color: #ffffff;
  ...
  handlerTrajectoryList_more() {
    ...
    // vm.chart.trajectoryDetailSource = this.checkedSource;
  }

  handlerTrajectoryDetail_show($item: any) {
    const vm = $scope.vm;

    $scope.$TrajectoryDetail.emit($item),
      (vm.chart.trajectoryDetailSource = $item),
      (vm.chart.trajectoryDetailLoading = true);
    this.checkedSource = $item;
  }
  ...
  handlerTrajectoryDetail_close() {
    ...
    if (vm.chart.trajectoryDetail) {
      (vm.chart.trajectoryDetail = []),
        (vm.chart.trajectoryDetailSource = null),
        (vm.chart.trajectoryDetailLoading = false);
    }
  }
