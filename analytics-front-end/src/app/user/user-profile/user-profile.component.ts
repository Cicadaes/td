...
export class UserProfileComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  public page: any = 1; //用户档案指标分析按照下拉框数据当前页数
  longRangeSearchLoading: boolean = true; //用户档案指标分析按照下拉框数据loading显示?
  constructor(private injector: Injector, private userProfileService: UserProfileService) {
    super(injector);
      ...
      $scope.userProfileService.req_profileDetail(finalParam).subscribe((response2: any) => {
        ...
        if (a && a.length === 0) {
          $scope.notification.create(
            'error',
            '提示',
            '暂未对用户属性进行配置，请在管理-用户配置-用户洞察配置-基础属性配置中进行配置，如果没权限，请联系管理员。'
          );
        }
  ...
  getEventDate() {
    ...
    $scope.userProfileService.req_eventDate(params).subscribe((response: any) => {
      let a = [],
        b;
  ...
  initIndexAnalysis_eventSelect($next?: any) {
    ...
    $scope.userProfileService.req_eventList(param).subscribe((response: any) => {
      let a, b, i;
      for (a = [], i = 0; (b = response.data[0][i]); i++) {
        a.push({
          label: b.eventName,
          value: b.eventId
        });
      ...
      if (response.data[1]['total'] > $scope.indexAnalysis.eventSelect.length) {
  ...
  onSearchEvent(data: any) {
    this.page = 1;
    this.req_eventList(1, data, true);
  }
  /**
   * 用户档案指标分析按照下拉框数据
   */
  req_eventList(page: number = 1, search: any = '', _search: boolean = false) {
    if (page == 1 && _search) {
      this.longRangeSearchLoading = true;
      $scope.indexAnalysis.eventSelect = [];
    }
    const date1 = $scope.indexAnalysis.dateRange[0];
    const date2 = $scope.indexAnalysis.dateRange[1];
    const param = {
      productId: $scope.productId,
      offset: $scope.offset,
      startDate: this.commonService.getDateStr(date1),
    ...
    $scope.userProfileService.req_eventList(param).subscribe((response: any) => {
      let a, b, i;
      if (response.data[0] && response.data[0].length) {
        for (a = [], i = 0; (b = response.data[0][i]); i++) {
          a.push({
            label: b.eventName,
            value: b.eventId
      ...
      if (_search) {
        ...
        $scope.indexAnalysis.eventSelect = [...$scope.indexAnalysis.eventSelect, ...a];
      }
      if (response.data[1]['total'] > $scope.indexAnalysis.eventSelect.length) {
        this.longRangeSearchLoading = true;
      } else {
  ...
  initIndexAnalysis_eventPropSelect($next?: any) {
    if ($scope.indexAnalysis.eventSelected != null) {
      $scope.userProfileService
        .req_eventPropertyList($scope.indexAnalysis.eventSelected.value)
  ...
  initBehaviorTrajectory_trajectoryList($next?: any) {
    ...
    $scope.userProfileService.req_eventHistory(p).subscribe((response: any) => {
      ...
      if ($scope.behaviorTrajectory.trajectoryPageIndex > 1 && a && a.length > 0) {
  ...
  handlerBehaviorTrajectory_trajectoryDetail($item: any) {
    ...
    params.accountOffset = $scope.accountOffset;
  ...
  // 用户档案-行为轨迹-显示更多
  handlerBehaviorTrajectory_trajectoryMore() {
  ...
  handlerBehaviorTrajectory_dateChange($data: any) {
    if ($data) {
      ...
      if ($data.loading) {
        ...
        $scope.behaviorTrajectory.trajectoryList = [];
        $scope.initBehaviorTrajectory_trajectoryList();
  ...
  ngOnDestroy() {
    ...
    switch (hash) {
      case '/user/user-insight':
        localStorage.setItem('UserInsightInto', 'Y');
        break;
      case '/user/user-insight/list':
        localStorage.setItem('UserInsightInto', 'Y');
        break;
      default:
