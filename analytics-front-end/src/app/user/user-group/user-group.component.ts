...
export class UserGroupComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  constructor(
    ...
    that._initAdvancedSearchParams();
  ...
  getList(page?: number) {
    ...
    that.page = page || 1;
    // that.advancedSearchList();
    that.advancedSearchList(that.page);
  }
        ...
        that.userGroupService.deleteCrowdById(data.id).subscribe((res: any) => {
          if (res.code === 200) {
            ...
            this.modalService.success({
              nzTitle: '提示',
              nzContent: `人群${data.name}删除成功`,
              nzOnOk: () => {
                this.handleCancel();
              }
            });
          } else {
            if (res.data && res.data.crowd) {
              this._checkTagCallback(res.data);
            } else {
  ...
  advancedSearch() {
    ...
    if (that.showAdvancedSearch) {
      that.advancedSearchParams.source = that.crowdSourceList[0].key;
      that.advancedSearchParams.status = that.statusList[0].key;
      that.advancedSearchParams.calcStatus = that.calcStatusList[0].key;
    }
    that.createDate = [];
    that.updateDate = [];
    that.page = 1;
  ...
  advancedSearchList(page?: number) {
    ...
    if (
      (that.advancedSearchParams['crowdCountEnd'] || that.advancedSearchParams['crowdCountEnd'] == '0') &&
      that.advancedSearchParams['crowdCountStart'] > that.advancedSearchParams['crowdCountEnd']
    ) {
    ...
    if (
      that.advancedSearchParams['crowdCountStart'] > 2147483647 ||
      that.advancedSearchParams['crowdCountEnd'] > 2147483647
    ) {
