package com.talkingdata.marketing.web.filter;
...
public class MenuFilter implements Filter{
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (user == null && principal != null && principal instanceof CasAuthenticationToken) {
            ...
            //得到当前用户
            try {
                user= UmRmiServiceFactory.getSecurityService().getUserByUmId(p.getUserDetails().getUsername() == null ? null : p.getUserDetails().getUsername());
        ...
        if(null != user) {
            // 获取用户资源，从login逻辑借鉴而来
            boolean isInited = session.getAttribute("isInited") == null ? false : (Boolean) session.getAttribute("isInited");
            int num = 0;
            //初始化菜单按钮，用户自定义type列表，用户拥有的角色，并放入session
            int tryTimes = 3;
            while (num < tryTimes && !isInited) {
                try {
                    session.setAttribute("isInited", true);
                    List<LinkedHashMap<String, Object>> list = UmRmiServiceFactory.getSecurityService().getAuthMenu(appcode, menu, user.getUmid(), apptaken);
                    List<LinkedHashMap<String, Object>> buttonlist = UmRmiServiceFactory.getSecurityService().getAuthMenu(appcode, button, user.getUmid(), apptaken);
                ...
                if(!StringUtils.isBlank(typeList)){
                    ...
                    for(String type:types){
                        List<LinkedHashMap<String, Object>> typelist = UmRmiServiceFactory.getSecurityService().getAuthMenu(appcode,type,user.getUmid(),apptaken);
                ...
                //用户拥有的角色放入session
                List<Role> roleList= UmRmiServiceFactory.getSecurityService().getRolesByUmid(user.getUmid(),appcode);
