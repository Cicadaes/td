...
public class HttpClientUtil {
	...
	private static ResponseHandler responseHandler = new ResponseHandler() {
        ...
        public String handleResponse(HttpResponse response) throws ClientProtocolException, IOException {
			...
			if (!(response.getStatusLine().getStatusCode() == 200 || response.getStatusLine().getStatusCode() == 201)) {
				String errBody = null;
				if (entity != null) {
					String charset = ContentType.getOrDefault(entity) == null ? CHARSET_GBK : ContentType.getOrDefault(entity).getCharset().name();
	...
	public static String post(String url, String authorization, HttpEntity httpEntity, String charset) {
		...
		if (!authorization.isEmpty()) {
			hp.addHeader(HttpUtil.AUTHORIZATION_KEY, authorization);
		}
		// 发送请求，得到响应
		String responseStr = null;
		try {
	 ...
	 * @return 响应消息
	 */
	public static String postReal(String url, String authorization, Map params, String charset) {
		if (url == null || url.trim().length() == 0) {
			return null;
		}
		List <BasicNameValuePair> basicNameValuePairs = getPostParamsList(params);
		...
		try {
			if (charset == null || charset.trim().length() == 0) {
				formEntity = new UrlEncodedFormEntity(basicNameValuePairs);
			} else {
				formEntity = new UrlEncodedFormEntity(basicNameValuePairs, charset);
			}
		} catch (UnsupportedEncodingException e) {
			throw new RuntimeException("不支持的编码集", e);
		}
		HttpPost hp = new HttpPost(url);
		// hp.setEntity(formEntity);
		hp.addHeader(HTTP.CONTENT_TYPE, "application/x-www-form-urlencoded;charset=UTF-8");
		hp.addHeader(HttpUtil.AUTHORIZATION_KEY, authorization);
		// 发送请求，得到响应
		String responseStr = null;
		try {
	...
	public static String post(String url, Map params, String charset, final URL keystoreUrl, final String keystorePassword, final URL truststoreUrl,
		...
		try{
			...
			CloseableHttpClient httpclient = getDefaultSSLHttpClient(charset,keystoreUrl,keystorePassword,truststoreUrl,truststorePassword);
	...
	public static String postWithJson(String url, String json) {
		CloseableHttpClient httpclient = getDefaultHttpClient(StandardCharsets.UTF_8.name());
		HttpPost hp = new HttpPost(url);
		hp.addHeader(HTTP.CONTENT_TYPE, APPLICATION_JSON);

		StringEntity se = null;
		try {
			se = new StringEntity(json, StandardCharsets.UTF_8);
...
/*		httpclient.getParams().setParameter(CoreProtocolPNames.PROTOCOL_VERSION, HttpVersion.HTTP_1_1);
	...
	private static CloseableHttpClient getDefaultSSLHttpClient(String charset,final URL keyStoreUrl,String keyStorePwd,final URL truststoreUrl,String trustStorePwd)
		...
		SSLConnectionSocketFactory sslConnSocketFactory
				= new SSLConnectionSocketFactory(buildSslContext(keyStoreUrl,keyStorePwd,truststoreUrl,trustStorePwd));
	 ...
	 * @throws KeyStoreException
	 */
	private static SSLContext buildSslContext(final URL keyStoreURL,String keyStorePwd,
			final URL trustStoreURL,String trustStorePwd) throws UnrecoverableKeyException, NoSuchAlgorithmException, KeyStoreException, KeyManagementException, KeyStoreException, IOException, CertificateException {

		SSLContextBuilder customContext = SSLContexts.custom();
		customContext.loadTrustMaterial(trustStoreURL, trustStorePwd.toCharArray());
		customContext.loadKeyMaterial(keyStoreURL,trustStorePwd.toCharArray(),keyStorePwd.toCharArray());
	 ...
	 * @throws NoSuchAlgorithmException
	 */
	public static SSLContextBuilder ignoreUnverifiedSSL(SSLContextBuilder customContext) throws KeyStoreException, NoSuchAlgorithmException {
