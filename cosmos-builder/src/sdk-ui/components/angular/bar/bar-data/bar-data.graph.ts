...
export class BarDataGraph extends BaseDataGraph {
    ...
    beforeRequest(queryChartParam: any, scope: string, linkageData?: any) {
        ...
        let param = {
            ...
            orderBy: this.buildRequestOrderBy(queryChartParam, linkageData),
            sumMetric: true,
            settingZero: true, //补数
            limit: {
                page: 1,
        ...
        if (queryChartParam["dimensions"].length == 2) {
            if (!param["orderBy"]) {
                param["orderBy"] = [];
            }
            param["orderBy"].unshift(
                { field: queryChartParam["dimensions"][0].value, order: "asc" })
        }
        if (param["orderBy"] && param["orderBy"].length == 0) {
            delete param["orderBy"];
        }
        ...
        if (queryChartParam["count"]) {
            param["barTotal"] = queryChartParam["count"];
        }
