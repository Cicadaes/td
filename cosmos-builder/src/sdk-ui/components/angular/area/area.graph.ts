...
export class AreaGraph extends TdBaseGraph {
    decimals: any = "auto";
    abscissa: boolean;
    dimensiondate: boolean;
    originData: any = [];
    constructor(data: any = {}) {
        ...
        if (data && data["data"] && data["data"][0] && data["data"][0].length > 0) {
            ...
            this.decimals = (data.style && data.style.line && data.style.line.decimals);
            data && data.data && data.data.length && this.setDataPrecision({ decimals: this.decimals }, data.data[0]["data"]);
     ...
     * @return {[type]}             [description]
     */
    setDataType(DATA_TYPE: number = 0, gptype: string = 'line') {
        if (!DATA_TYPE) {
            this.series.push({
                data: [],
                type: gptype
    ...
    private setLineItemStyle(data: any = {}, missingData: string = "breakValue") {
        let colorLen = this.color.length;
        let that = this;
        this.series && this.series.forEach((item: any, i: any) => {
            item && Object.assign(item, {
                ...
                areaStyle: {
                    normal: {},
                    color: this.color[i % colorLen] || this.color[0]
                },
                itemStyle: {
                    normal: {
                        label: {
   ...
   * @param obj 精度
   */
    setDataPrecision(obj: any, data: any) {
        if (obj && obj.decimals != "auto") {
            for (let item of data) {
                for (let key in item) {
                    if (String(item[key]).indexOf('%') >= 0) {
                        item[key] = Number(item[key].split('%')[0]).toFixed(obj.decimals - 0) + "%";
