...
export class MarketingPlanComponent extends BaseComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    constructor(private marketingPlanService: MarketingPlanService,
    ...
    queryActiveUsers(queryParam: any) {
        const that = this;
        that.marketingPlanService.queryActiveUsers({
            ...
            that.activityChart = _.cloneDeep(that.activityChartOption);
        });
    }
    ...
    beforeMonth() {
        ...
        this.queryParams.startTimeBegin =
            `${this.yearList[0].year}-${this.monthList[0]}-1`;
        // new Date((moment()
    ...
    querySclPush(queryParam: any) {
        const that = this;
        let param = _.cloneDeep(queryParam);
