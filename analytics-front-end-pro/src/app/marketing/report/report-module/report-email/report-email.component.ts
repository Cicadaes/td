...
@Component({
    ...
    styleUrls: ['./report-email.component.less']
})
export class ReportEmailComponent implements OnInit, OnChanges {
    // @Input() cycleTime: any;
    // @Input() right: any;
    // @Input() segmentId: any;
    ...
    ngOnInit() {
        ...
        this.chartTab = [
            {
                name: '发送成功数',
                value: 0,
                key: 'sum_td_edm_send_success'
            },
            {
                name: '打开邮件数',
                value: 0,
                key: 'sum_td_edm_open_count'
            },
            {
                name: '点击链接数',
                value: 0,
                key: 'sum_td_edm_click_count'
            }
        ];
        ...
        this.getEdmTrend(this.reportObj.segmentId, this.timeType);
        this.initEchart();
    }
     ...
     */
    getEdmOverview() {
        this.reportService.getEdmOverview(this.reportObj.segmentId).subscribe((response) => {
            if (response.code === 200) {
                const data = response.data[this.reportObj.segmentId];
                let successRate, failRate, unknowRate, nvalidAddressRate, sufficienRate, closedAccountRate;
                let successCount = 0, failCount = 0, unknowCount = 0;
                if (response.data && response.data[this.reportObj.segmentId] && data['sum_td_edm_send_count']) {
                    // 成功率
                    successRate = ((data['sum_td_edm_send_success'] / data['sum_td_edm_send_count']) * 100).toFixed(0) + '%';
                    // 失败率
                    failRate = ((data['sum_td_edm_send_fail'] / data['sum_td_edm_send_count']) * 100).toFixed(0) + '%';
                    // 未知率
                    unknowRate = ((data['sum_td_edm_send_unknow'] / data['sum_td_edm_send_count']) * 100).toFixed(0) + '%';
                    // 无效地址
                    nvalidAddressRate = ((data['sum_td_edm_send_invalid_address'] / data['sum_td_edm_send_count']) * 100).toFixed(1) + '%';
                    // 账户关闭
                    closedAccountRate = ((data['sum_td_edm_send_closed_account'] / data['sum_td_edm_send_count']) * 100).toFixed(1) + '%';
                    // 账户余额不足
                    sufficienRate = ((data['sum_td_edm_send_not_sufficient_funds'] / data['sum_td_edm_send_count']) * 100).toFixed(1) + '%';
                    successCount = data['sum_td_edm_send_success'];
                    failCount = data['sum_td_edm_send_fail'];
                    unknowCount = data['sum_td_edm_send_unknow'];
                    ...
                    unknowRate = 0 + '%';
                    // 无效地址
                    nvalidAddressRate = 0.0 + '%';
                    // 账户关闭
                    closedAccountRate = 0.0 + '%';
                    // 账户余额不足
                    sufficienRate = 0.0 + '%';
                }

                this.emailContentList = [
                    {
                        name: '成功',
                        status: '1',
                    ...
                    },
                    {
                        ...
                        error: [
                            {
                                name: '邮箱地址无效',
                                value: nvalidAddressRate
                            },
                            {
                                name: '账户关闭',
                                value: closedAccountRate
                            }
                            ,
                            {
                                name: '账户余额不足',
    ...
    getContribute(segmentId: any, campaignId: any) {
        this.reportService.getContribute(segmentId, campaignId).subscribe((response) => {
            if (response.code === 200) {
                ...
                console.log( this.contributeList.length);
            } else {
                this.notification.create('warning', '错误提示', response.message);
            }
        });
    ...
    getEdmTrend(segmentId: any, statDimension: any) {
        this.reportService.getEdmTrend(segmentId, statDimension).subscribe((response) => {
            if (response.code === 200) {
    ...
    changeTrendCahrtData(response: any, itemkey: any) {
        ...
        if (this._total > 0) {
            let clickTotal = 0, openTotal = 0, sendTotal = 0;
                    ...
                    for (const i in response.data[key]) {
                        if (response.data[key].hasOwnProperty(i)) {
                            if (this.chartTabKey) {
                                ...
                                if (key === 'sum_td_edm_send_success') {
                                    sendTotal += response.data[key][i];
                                }

                                if (key === 'sum_td_edm_open_count') {
                                    openTotal += response.data[key][i];
                                }

                                if (key === 'sum_td_edm_click_count') {
    ...
    checkOneTime(item: any) {
        ...
        this.getEdmTrend(this.reportObj.segmentId, item.type);
    }
    ...
    download(type: any) {
        if (type === 'error') {
            this.reportService.downLoadEdmOver(this.reportObj.segmentId).subscribe((response: any) => {
                if (response && response.code !== 200) {
                    this.notification.create('warning', '错误提示', response.message);
                }
            });
        } else {
            this.reportService.downLoadEdmTrend(this.reportObj.segmentId, this.timeType).subscribe((response: any) => {
                if (response.code !== 200) {
                    this.notification.create('warning', '错误提示', response.message);
                }
            });
