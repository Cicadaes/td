    ...
    constructor(private activityCenterService: ActivityCenterService,
        ...
        window.addEventListener('message', (event: any) => {
            if (event.data && typeof event.data === 'string') {
                ...
                if (data && data.eventInfo && data.eventType === 'dwClose') { // 关闭dw窗口
                    // const list = data.eventInfo.data;
                    $scope.modalRef.close();
                    $scope.initContentData();
                }
                if (data && data.eventInfo && data.eventType === 'dwPre') { // dw窗口选择“上一步”
                    $scope.modalRef.close();
...
//        }
    ...
    handlerDwPage(type: any, dwSegmentId: number) {
        ...
        if (type === 'create') {
            const sclData = $scope.scl.vm;
            url = `${this.dwUrlPrefix}${this.dwUrl.create}?winId=${this.guid()}&name=${sclData.launchNameValue}&productId=${this.productId}&token=${localStorage.getItem('token')}`;
            if (sclData.launchDescriptionValue) {
                url += `&desc=${sclData.launchDescriptionValue}`;
            }
        } else if (type === 'detail') {
            url = `${this.dwUrlPrefix}${this.dwUrl.detail}?winId=${this.guid()}&id=${dwSegmentId}&token=${localStorage.getItem('token')}`;
        } else if (type === 'report') {
            url = `${this.dwUrlPrefix}${this.dwUrl.report}?winId=${this.guid()}&id=${dwSegmentId}&token=${localStorage.getItem('token')}`;
        }
