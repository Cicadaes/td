...
export class ActivityCenterAppPushComponent extends BaseComponent {
    ...
    constructor(private activityCenterAppPushService: ActivityCenterAppPushService,
    ...
    addCustomParam() {
        ...
        if (that.appPush[that.seletABIndex].extendAttr[length - 1]['key'] && that.appPush[that.seletABIndex].extendAttr[length - 1]['value']) {
    ...
    buildSendTime(value: any) {
        ...
        const sendTime = `${this.sendTime.getFullYear()}-${this.sendTime.getMonth() + 1}-${this.sendTime.getDate()} ${hour}:${minute}:00`;
    ...
    checkParam() {
        ...
        for (let i = 0; i < length; i++) {
            if (!pushData[i].action) {
                ...
                if (pushData[i].digitalAngle) { // 数字角标
