...
export class AppPushManageComponent extends BaseComponent implements OnInit, OnChanges {
    ...
    constructor(private appPushManageService: AppPushManageService,
...
//                };
    ...
    upload(tp: number) {
        ...
        // 加提示file为空
        if (this.file) {
            this.appPushManageService.updateIOS(this.beforUpload(this.file, this.pwd), this.appKey, tp).subscribe((response) => {
                if (response.code === 200) {
                    this.testTipShow = false;
                    this.proTipShow = false;
                    if (tp === 0) {
