...
public class BitmapZkCoordinator implements BitmapPartitionCoordinator {
    ...
    Map<Partition, BitmapPartitionManager> _managers = new HashMap();
    List<BitmapPartitionManager> _cachedList;
    ...
    public BitmapZkCoordinator(DynamicPartitionConnections connections, Map stormConf, SpoutConfig spoutConfig, ZkState state, int taskIndex, int totalTasks, String topologyInstanceId) {
        this(connections, stormConf, spoutConfig, state, taskIndex, totalTasks, topologyInstanceId, buildReader(stormConf, spoutConfig));
    }

    public BitmapZkCoordinator(DynamicPartitionConnections connections, Map stormConf, SpoutConfig spoutConfig, ZkState state, int taskIndex, int totalTasks, String topologyInstanceId, DynamicBrokersReader reader) {
    ...
    @Override
    public List<BitmapPartitionManager> getMyManagedPartitions() {
    ...
    public void refresh() {
        try {
            ...
            for (Partition id : deletedPartitions) {
            	BitmapPartitionManager man = _managers.remove(id);
                man.close();
            }
            LOG.info(taskId(_taskIndex, _totalTasks) + "New partition managers: " + newPartitions.toString());

            for (Partition id : newPartitions) {
            	BitmapPartitionManager man = new BitmapPartitionManager(_connections, _topologyInstanceId, _state, _stormConf, _spoutConfig, id);
        ...
        _cachedList = new ArrayList<BitmapPartitionManager>(_managers.values());
        LOG.info(taskId(_taskIndex, _totalTasks) + "Finished refreshing");
    }

    @Override
    public BitmapPartitionManager getManager(Partition partition) {
