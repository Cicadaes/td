...
calcObjectCode="MetricFactTask"
calcObjectName="实时累计客流"
function usage() {
...
# 实时累计客流 MetricFactTask
echo "<<etl exec>> MetricFactTask exec starting..."

java -classpath .:$basedir/../libs/* td.enterprise.wanalytics.etl.task.lz.MetricFactTask  --date ${date} --hour ${hour} --limit ${limit} --reRun ${rerun}

result=$?
if [ $result -eq 1 ]; then
  echo "<<etl exec>> MetricFactTask calculate failed!!!"
...
java -classpath .:$basedir/../libs/* td.enterprise.wanalytics.etl.task.group.AggregationMetricFactTask --date ${date} --hour ${hour} --limit ${limit} --reRun ${rerun} --parentProjectId ${parentProjectId}

result=$?
if [ $result -eq 1 ]; then
  echo "<<etl exec>> AggregationMetricFactTask calculate failed!!!"
  #更新计算对象日志状态为异常，后端会自动获取异常信息
  finishCalcObjectLogWithExcetpion $calcObjectLogId ""
  echo "$calcObjectName($calcObjectCode) exit with exception!!!"
  exit 1
...
echo "<<etl exec>> MetricFactTask exec finished."
exit 0
