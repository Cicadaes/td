...
# 实时天客流 MetricDayTask
echo "<<etl exec>> MetricDayTask exec starting..."

java -classpath .:$basedir/../libs/* td.enterprise.wanalytics.etl.task.lz.MetricDayTask --date ${date}

result=$?
if [ $result -eq 1 ]; then
  echo "<<etl exec>> MetricDayTask calculate failed!!!"
...
java -classpath .:$basedir/../libs/* td.enterprise.wanalytics.etl.task.group.AggregationDataByDayTask --date ${date} --type ${type} --parentProjectId ${parentProjectId}
if [ $? -ne 0 ];then
  content="汇总失败"
...
echo "<<etl exec>> MetricDayTask exec finished."
exit 0
