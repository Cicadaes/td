...
calcObjectCode="copyLogToES_${runDate}"
...
##跳出客流计算
java -classpath .:$basedir/../libs/* td.enterprise.wanalytics.etl.task.lz.JumpTask  --date ${runDate}

result=$?
if [ $result -eq 1 ]; then
  echo "跳出客流失败JumpTaskError"
  exit 1
fi
...
#ComputeTimesAndDurationTask
bash -x ${basedir}/ComputeTimesAndDurationTask.sh --runDate "${runDate}" --schedulerTaskLogId ${schedulerTaskLogId}
result=$?
if [ $result -eq 1 ]; then
  echo "<<etl exec>> Exception: ComputeTimesAndDurationTask calculate failed!!!"
  exit 1
fi


#到访周期
bash -x ${basedir}/VisitCycle.sh   --runDate "${runDate}"  --schedulerTaskLogId "${schedulerTaskLogId}"
result=$?
if [ $result -eq 1 ]; then
  echo "<<etl exec>> Exception:VisitCycleTask calculate failed!!!"
  exit 1
fi
...
#所有指标汇总更新
bash -x ${basedir}/AggregationDataByDayTask.sh --runDate "${tomorrow}" --type "day" --parentProjectId "-1" --schedulerTaskLogId ${schedulerTaskLogId}
result=$?
if [ $result -eq 1 ]; then
