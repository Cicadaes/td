...
calcObjectCode="AggregationMetricFactTask_${runDate}"
calcObjectName="汇总计算"
...
java -classpath .:$basedir/../libs/* td.enterprise.wanalytics.etl.task.group.AggregationMetricFactTask --date ${runDate} --hour ${hour} --limit ${limit} --reRun ${reRun} --parentProjectId ${parentProjectId}
if [ $? -ne 0 ];then
  content="汇总失败"
