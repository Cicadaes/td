...
calcObjectCode="MetricDayActiveTask"
...
# 实时天客流 MetricDayActiveTask
echo "<<etl exec>> MetricDayActiveTask exec starting..."

java -classpath .:$basedir/../libs/* td.enterprise.wanalytics.etl.task.lz.MetricDayActiveTask --factDate $date

result=$?
if [ $result -eq 1 ]; then
  echo "<<etl exec>> MetricDayActiveTask calculate failed!!!"
...
echo "<<etl exec>> MetricDayActiveTask exec finished."
exit 0
