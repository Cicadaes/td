...
calcObjectCode="OfflineProjectMacDateJob"
...
# 实时天客流 OfflineProjectMacDateJob
echo "<<etl exec>> OfflineProjectMacDateJob exec starting..."
...
echo "OfflineProjectMacDateJob RunDate ${date}"
...
result=$?
if [ $result -eq 1 ]; then
  echo "<<etl exec>> OfflineProjectMacDateJob calculate failed!!!"
...
echo "<<etl exec>> OfflineProjectMacDateJob exec finished."
exit 0
