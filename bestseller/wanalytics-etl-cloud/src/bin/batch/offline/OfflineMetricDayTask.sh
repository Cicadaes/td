#!/usr/bin/env bash
#OfflineMetricDayTask
basedir=$(cd "$(dirname "$0")"; pwd)

source $basedir/../env.sh
source $basedir/../func.sh

calcObjectCode="OfflineMetricDayTask"
...
# 实时天客流 OfflineMetricDayTask
echo "<<etl exec>> OfflineMetricDayTask exec starting..."

getYesterday $date

date=$yesterday
echo "OfflineMetricDayTask RunDate ${date}"

java -classpath .:$basedir/../../libs/* td.enterprise.wanalytics.etl.task.lz.MetricDayTask --date ${date}

result=$?
if [ $result -eq 1 ]; then
  echo "<<etl exec>> OfflineMetricDayTask calculate failed!!!"
...
echo "<<etl exec>> OfflineMetricDayTask exec finished."
exit 0
