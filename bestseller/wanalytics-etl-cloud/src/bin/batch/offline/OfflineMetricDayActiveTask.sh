#!/usr/bin/env bash
#OfflineMetricDayActiveTask
basedir=$(cd "$(dirname "$0")"; pwd)

source $basedir/../env.sh
source $basedir/../func.sh

calcObjectCode="OfflineMetricDayActiveTask"
calcObjectName="实时天客流"
function usage() {
...
# 实时天客流 OfflineMetricDayActiveTask
echo "<<etl exec>> OfflineMetricDayActiveTask exec starting..."

getYesterday $date

date=$yesterday
echo "OfflineMetricDayActiveTask RunDate ${date}"

java -classpath .:$basedir/../../libs/* td.enterprise.wanalytics.etl.task.lz.MetricDayActiveTask --factDate $date

result=$?
if [ $result -eq 1 ]; then
  echo "<<etl exec>> OfflineMetricDayActiveTask calculate failed!!!"
...
echo "<<etl exec>> OfflineMetricDayActiveTask exec finished."
exit 0
