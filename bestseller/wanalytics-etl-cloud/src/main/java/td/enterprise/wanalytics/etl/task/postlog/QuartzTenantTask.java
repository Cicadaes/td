...
public class QuartzTenantTask {

    private static final Logger logger = Logger.getLogger(QuartzTenantTask.class);

    public static void main(String[] args) {
        try{

            Options options = new Options();
            options.addOption("c", "configInterval", true, "租户接收配置更新数据间隔");
            ...
            try {
                ...
                if (days <= 31) {
                    QuartzUtil.addConfigJob(configName, configJob, quartzTime, true);
                    logger.info("---------------------addConfigJob:" + quartzTime);
                } else {
                    QuartzUtil.addConfigJob(configName, configJob, "0 0 0 31 * ?", true);
