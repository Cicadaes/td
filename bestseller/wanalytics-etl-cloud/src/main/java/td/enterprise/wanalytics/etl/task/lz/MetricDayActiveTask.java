 ...
 */
@Slf4j
public class MetricDayActiveTask {
    ...
    public static void main(String[] args) {
        ...
        String dateStr = "2017-11-16";

        Options options = new Options();
        options.addOption("d", "factDate", true, "实时客流统计日期");

        CommandLineParser parser = new DefaultParser();
        CommandLine line;
        try {
            line = parser.parse(options, args);
            dateStr = line.getOptionValue("factDate");
        ...
        try {
            ...
            e.printStackTrace();
            log.error("metricDayActiveError:", e);
        }
        ...
        log.info("MetricDayActiveTask:{}", (end - start) + " Minlls");
    ...
    private static Set<String> queryMetricDay(String dateStr) throws SQLException {
        String sql = "select project_id from TD_METRIC_DAY_ACTIVE where date='" + dateStr + "' and project_type=1";
    ...
    public static void dayBatchInsert(String dateStr, Map<String, Project> allProjects) throws SQLException {
        ...
        String insertSql = "insert into TD_METRIC_DAY_ACTIVE "//
        ...
        String updateSql = "update TD_METRIC_DAY_ACTIVE "//
                + "set active_new_users=?,active_old_users=?,active_users=?,stay_new_users=?,stay_old_users=?,stay_users=?,"//
                + "front_users=?,jump_users=?," //
                + "high_active_users=?,middle_active_users=?,low_active_users=?,sleep_active_users=?,"
                + "high_stay_users=?,middle_stay_users=?,low_stay_users=?,sleep_stay_users=?,update_time=?,tenant_id=?" // 
        ...
        for (Map.Entry<String, Project> entry : allProjects.entrySet()) {
            ...
            if (dateMetrictSet.contains(projectId)) {
                ...
                updateps.setString(18, entry.getValue().getTenantId());

                //where
                updateps.setString(19, projectId);
                updateps.setString(20, dateStr);
                ...
                insertps.setString(31, p.getCityCnName());
                insertps.setString(32, p.getTenantId());
