...
/**
 * 菜单初始化 获取用户信息 获取资源类型信息，把权限信息放入session <br/> <p> <p> session描述 <br/> key:user 用户在um上维护的详细信息 <br/>
 * key:roleList 当前登陆用户在当前应用系统的角色信息 <br/> key:menuList 系统的菜单详细信息 <br/> key:UserACL 系统的按钮信息 <br/>
 * key:authList 登陆用户的权限信息 <br/> <p> <p> application描述 <br/> appcode：系统编码 <br/> apptaken：系统令牌 <br/>
 * <p> filter的用户描述如下（上面的配置信息在web.xml中配置，所实现的全部功能都有配置）	 <br/> 配置可在部署项目的classpath下的文件sysConfig.properties中进行覆盖配置
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
    ...
    if (user == null && principal != null) {
      ...
      if (principal instanceof CasAuthenticationToken) {
        ...
        username = p.getUserDetails().getUsername();
      ...
      //得到当前用户
      try {
        user = UmRmiServiceFactory.getSecurityService().getUserByUmId(username);
        if (user == null) {
          throw new td.enterprise.common.exception.BusinessException("获取用户失败");
        }
        ...
        try {
          throw new td.enterprise.common.exception.BusinessException("获取用户失败");
        } catch (td.enterprise.common.exception.BusinessException e1) {
          e1.printStackTrace();
        }
      } catch (td.enterprise.common.exception.BusinessException e) {
        e.printStackTrace();
      }
      //得到当前租户
      try {
      ...
      session.setAttribute(SessionConstant.CURRENT_TENANT_ID, user.getTenantId());
    ...
    while (user != null && num < 3 && !isInited) {
      try {
