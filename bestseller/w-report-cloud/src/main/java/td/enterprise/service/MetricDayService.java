...
/**
 * <br> <b>功能：</b>排行榜表 MetricDayService<br> <b>作者：</b>code generator<br> <b>日期：</b> 2017-09-22 <br>
 * <b>版权所有：</b>Copyright(C) 2015, Beijing TendCloud Science & Technology Co., Ltd.<br>
 */
@Service("metricDayService")
@Transactional(value = "transactionManager", readOnly = false, propagation = Propagation.REQUIRED, rollbackFor = Throwable.class)
public class MetricDayService extends BaseService<MetricDay> {

  private final static Logger logger = Logger.getLogger(MetricDayService.class);
  private final static SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
  private final static Calendar calendar = Calendar.getInstance();

  @Autowired
  public MetricDayDao dao;

  public MetricDayDao getDao() {
    return dao;
  }


  private String getMinusDate(String date, int minusDay) {
    try {
      calendar.setTime(format.parse(date));
      calendar.add(Calendar.DAY_OF_YEAR, -1 * minusDay);
  ...
  public List<MetricDayVM> queryByListWithChain(MetricDayPage page) {
    page.setUmid(UserInfoUtil.getUser().getUmid());
    ...
    if (page.getStartDate() == null || "".equals(page.getStartDate())) {
      ...
      page.setStartDate(format.format(new Date(calendar.getTimeInMillis() - 1000 * 60 * 60 * 24)));
      page.setEndDate(format.format(new Date(calendar.getTimeInMillis() - 1000 * 60 * 60 * 24)));
    }
...
//        要环比的日期差值（先转换为毫秒，再相减，然后除以每天的毫秒数）
    ...
    try {
      dayMinus =
          (int) (format.parse(page.getEndDate()).getTime() - format.parse(page.getStartDate())
...
//      设置要查询的环比日期的起止值
    page.setStartDate(getMinusDate(page.getStartDate(), dayMinus));
    page.setEndDate(getMinusDate(page.getEndDate(), dayMinus));
...
//            新客占比
        Object[] obj_7 = getRate(currentList.get(k).getActiveUserNewRate(),
            historyMetricDayVM.getActiveUserNewRate());
...
//            老客占比
        Object[] obj_8 = getRate(currentList.get(k).getActiveUserOldRate(),
            historyMetricDayVM.getActiveUserOldRate());
...
//         高活跃客占比
        Object[] obj_9 = getRate(currentList.get(k).getHighRate(),
            historyMetricDayVM.getHighRate());
        currentList.get(k).setHighRateChainRate((Double) obj_9[0]);
        currentList.get(k).setHighRateChainRateFlag((String) obj_9[1]);

//         中活跃客占比
        Object[] obj_10 = getRate(currentList.get(k).getMiddleRate(),
            historyMetricDayVM.getMiddleRate());
...
//          沉睡客占比
        Object[] obj_12 = getRate(currentList.get(k).getSleepRate(),
            historyMetricDayVM.getSleepRate());
        currentList.get(k).setSleepRateChainRate((Double) obj_12[0]);
        currentList.get(k).setSleepRateChainRateFlag((String) obj_12[1]);

//            次均停留时长
        Object[] obj_13 = getRate(currentList.get(k).getStayDurationPerTime(),
            historyMetricDayVM.getStayDurationPerTime());
...
//            件单价
        Object[] obj_20 = getRate(currentList.get(k).getSingularPrice(),
            historyMetricDayVM.getSingularPrice());
