...
import com.talkingdata.wifianalytics.offline.compute.dao.OffLineStayNewUserDayCounterDao;
import com.talkingdata.wifianalytics.offline.compute.dao.StayNewUserDayCubeDao;
 ...
 * @author junmin.li
 */
public class OffLineStayNewUserTask extends OffLineTask {
    private static Logger logger = Logger.getLogger(OffLineStayNewUserTask.class);

    private static StayNewUserDayCubeDao stayNewUserDayCubeDao = new StayNewUserDayCubeDao();
    
    public static OffLineStayNewUserDayCounterDao offLineStayNewUserDayCounterDao = new OffLineStayNewUserDayCounterDao();
    
    OffLineStayNewUserTask(String tenant_id, int project_id, String date, int dataInterval) {
        super(tenant_id, project_id, date, dataInterval);
    }

    @Override
    public void run() {
        logger.info(TaskName.OFFLINE_STAY_NEW_USER_DAY_TASK.getName() + " begin.");
        ...
        List<BitmapDayCube> resultList = stayNewUserDayCubeDao.query(this.tenant_id, this.project_id, start, end);
        logger.debug(TaskName.OFFLINE_STAY_NEW_USER_DAY_TASK.getName() + " query data : " + resultList);
        if (!CollectionUtils.isEmpty(resultList)) {
            	...
            	if (offLineUserDayCounters != null && offLineUserDayCounters.size() != 0) {
            		offLineStayNewUserDayCounterDao.insert(offLineUserDayCounters);
				}
            	logger.info(TaskName.OFFLINE_STAY_NEW_USER_DAY_TASK.getName() + " success.");
            } else {
                logger.info(TaskName.OFFLINE_STAY_NEW_USER_DAY_TASK.getName() + " failed.");
            }
        logger.info(TaskName.OFFLINE_STAY_NEW_USER_DAY_TASK.getName() + " end.");
    }
}
