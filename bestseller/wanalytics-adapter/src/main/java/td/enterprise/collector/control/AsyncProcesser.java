...
import td.enterprise.task.TdThreadFactory;
...
public class AsyncProcesser {
    ...
    private void init() {
        ...
        execsIn = (ThreadPoolExecutor) Executors.newFixedThreadPool(ThreadConstants.IN_THREAD_SIZE, new TdThreadFactory("execsIn"));

        execsOut = (ThreadPoolExecutor) Executors.newFixedThreadPool(ThreadConstants.OUT_THREAD_SIZE, new TdThreadFactory("execsOut"));

        execsOutSch = (ScheduledThreadPoolExecutor) Executors.newScheduledThreadPool(ThreadConstants.OUT_SCHEDULE_THREAD_SIZE, new TdThreadFactory(
                "execsOutSch"));

        for (int i = 0; i < ThreadConstants.OUT_SCHEDULE_THREAD_SIZE; i++) {
            execsOutSch.scheduleAtFixedRate(new FSQueueReader(), 0, PERIOD, TimeUnit.NANOSECONDS);
        }
    ...
    public void restartOutExec(int poolSize) {
        ...
        ThreadConstants.OUT_THREAD_SIZE = poolSize;
        execsOut = (ThreadPoolExecutor) Executors.newFixedThreadPool(ThreadConstants.OUT_THREAD_SIZE);
    }
    ...
    public void startSchedule() {
        execsOutSch = (ScheduledThreadPoolExecutor) Executors.newScheduledThreadPool(ThreadConstants.OUT_SCHEDULE_THREAD_SIZE);
        for (int i = 0; i < ThreadConstants.OUT_SCHEDULE_THREAD_SIZE; i++) {
    ...
    public void restartScheduleOutExec(int period, int poolSize) {
        ...
        ThreadConstants.OUT_SCHEDULE_THREAD_SIZE = poolSize;
        PERIOD = period;
        execsOutSch = (ScheduledThreadPoolExecutor) Executors.newScheduledThreadPool(ThreadConstants.OUT_SCHEDULE_THREAD_SIZE);
        for (int i = 0; i < ThreadConstants.OUT_SCHEDULE_THREAD_SIZE; i++) {
            execsOutSch.scheduleAtFixedRate(new FSQueueReader(), 0, PERIOD, TimeUnit.MILLISECONDS);
    ...
    public String stats() {
        ...
        sb.append("******** ThreadUtil ********").append("\r\n");
        sb.append(" isShutdown: ").append(ThreadUtil.getInstance().getPool().isShutdown()).append("\r\n");
        sb.append(" Core_Pool_Size: ").append(ThreadUtil.getInstance().getPool().getCorePoolSize()).append("\r\n");
        sb.append(" Maximum_Pool_Size: ").append(ThreadUtil.getInstance().getPool().getMaximumPoolSize()).append("\r\n");
        sb.append(" KeepAlive_Time: ").append(ThreadUtil.getInstance().getPool().getKeepAliveTime(TimeUnit.SECONDS)).append("\r\n");
        sb.append("\r\n");
        sb.append(" Active_count: ").append(ThreadUtil.getInstance().getPool().getActiveCount()).append("\r\n");
        sb.append(" Pool_Size: ").append(ThreadUtil.getInstance().getPool().getPoolSize()).append("\r\n");
        sb.append(" Largest_Pool_Size: ").append(ThreadUtil.getInstance().getPool().getLargestPoolSize()).append("\r\n");
        sb.append("\r\n");
        sb.append(" Task_Queue_Size: ").append(ThreadUtil.getInstance().getPool().getQueue().size()).append("\r\n");
        sb.append(" Completed_Task_count: ").append(ThreadUtil.getInstance().getPool().getCompletedTaskCount()).append("\r\n");
        sb.append(" Task_Count: ").append(ThreadUtil.getInstance().getPool().getTaskCount()).append("\r\n");
        sb.append("\r\n");
