import { Component, OnInit, Input, Output, EventEmitter, OnChanges, SimpleChanges } from '@angular/core';
import { FormBuilder, FormGroup, FormControl, Validators } from '@angular/forms';

import { ActivatedRoute } from '@angular/router'

import { AppTableService } from '../table/app-table.service';
import { DomSanitizer } from '@angular/platform-browser';

// 校验相关
import { CheckRegExp } from '../../../../@core/utils/validators/validator.service'
import { RegexpSService } from '../../../../@core/data/regexps.service'

@Component({
  selector: 'add-app-form',
  templateUrl: './add-app-form.component.html',
  styleUrls: ['./add-app-form.component.css'],
  providers: [FormBuilder]
})

export class AddAppFormComponent implements OnInit {
  @Input() needSubmit: boolean;
  @Input() app: any;
  @Input() errorFlag: any;
  @Input() errorFlagMax: any;
  @Output() onSubmit = new EventEmitter<any>();
  type: string = '1';
  status: boolean = true;
  validateForm: FormGroup;
  additionalAppIdSelect: any;
  isInitedForm: boolean = false;
  resultValue: any = {};
  ismultiple: boolean = true;
  imageErrorFlag: boolean = false;
  imageErrorFlagMax: boolean = false;
  @Output() resultData = new EventEmitter<any>();
  token: any;
  appIconFile: any = {
    list: [],
    number: 1,
    apiUrl: `${document.location.origin}/console-api/attachmentController/uploadImage`,
  };
  appIconFileMax: any = {
    list: [],
    number: 1,
    apiUrl: `${document.location.origin}/console-api/attachmentController/uploadImage`,
  };
  @Input() isEdit = false;

  // 所有的应用名称和代码
  public applistInfo: Array<any> = []

  // 字符串合法性校验
  private checkGenStr: any = CheckRegExp(this.regService.getGeneralstr())

  componentChange(value: any, fieldName: string) {
    if (this.checkHasFieldName(fieldName)) {
      this.validateForm.controls[fieldName].setValue(value);
    }
  }

  checkHasFieldName(fieldName: string) {
    let has = false;
    for (let o in this.validateForm.controls) {
      if (fieldName && fieldName == o) {
        has = true;
        break;
      }
    }
    return has;
  }

  _submitForm() {
    for (const i in this.validateForm.controls) {
      this.validateForm.controls[i].markAsDirty();
    }

    setTimeout(() => {
      this.resultData.emit(this.resultValue);
      this.onSubmit.emit(this.validateForm);
    }, 100);

  }
  constructor(
    private fb: FormBuilder,
    private regService: RegexpSService,
    private appListService: AppTableService,
    private domSanitizer: DomSanitizer,
    private route: ActivatedRoute
  ) {

  }

  selectSearchAdditionalAppId(value: any, fieldName: string) {
    this.resultValue.selectedIds = "";

    for (var i = 0; i < value.length; i++) {
      this.resultValue.selectedIds = this.resultValue.selectedIds + "," + value[i];
      //this.resultValue.selectedIds=this.resultValue.selectedIds +","+value[i].id;
    }
    this.componentChange(value, fieldName);
  }

  additionalAppIdSelectOptions(allOptions: any) {

  }


  initValidateForm() {
    if (this.validateForm) {
      return false;
    }
    if (this.type == '2') {

      this.validateForm = this.fb.group({
        // name: [null, [Validators.required, Validators.maxLength(256), this.checkRepeatName]],
        name: [null, [Validators.required, Validators.maxLength(256), Validators.pattern(/^\S.*\S$|^\S$/)], this.checkRepeatNameAs],
        status: [null, [Validators.required]],
        type: [null, [Validators.required]],
        // code: [null, [Validators.required, this.checkGenStr]],
        // code: [null, [Validators.required, this.checkGenStr, this.checkRepeatCode, Validators.maxLength(128), this.checkCNameRepeat]],
        code: [null, [Validators.required, Validators.maxLength(128), Validators.pattern(/^[0-9a-zA-Z_]{1,}$/)], this.checkRepeatCodeAs],
        version: [null, [Validators.required, Validators.maxLength(128)]],
        token: [null],
        uri: [null, [Validators.required, Validators.maxLength(256)]],
        additionalAppId: [null, [Validators.required]],
        provider: [null, [Validators.maxLength(256)]],
        desc: [null, [Validators.required, Validators.maxLength(256)]]
      });
    } else { //添加应用功能时用的

      this.validateForm = this.fb.group({
        // name: [null, [Validators.required, Validators.maxLength(256), this.checkRepeatName]],
        name: [null, [Validators.required, Validators.maxLength(256), Validators.pattern(/^\S.*\S$|^\S$/)], this.checkRepeatNameAs],
        status: [null, [Validators.required]],
        type: [null, [Validators.required]],
        code: [null, [Validators.required,  Validators.maxLength(128),,Validators.pattern(/^[0-9a-zA-Z_]{1,}$/)], this.checkRepeatCodeAs],
        version: [null, [Validators.required, Validators.maxLength(128)]],
        token: [null],
        uri: [null, [Validators.required, Validators.maxLength(256)]],
        provider: [null, [Validators.maxLength(256)]],
        desc: [null, [Validators.required, Validators.maxLength(256)]]
      });
    }

  }

  ngOnInit() {
    this.getApplist()

    this.token = this.guid();
    this.status = true;
    this.additionalAppIdSelect = {
      id: 3,
      fieldName: 'additionalAppId',
      fieldLabel: '关联主应用',
      fieldType: 'select',
      apiData: true,
      apiUrl: '/console-api/appController/queryListByName',
      apiParam: {},
      initValue: '',
      selectOptions: []
    };

    //this.appIconFile.list = [];

    this.appIconFile.list = [{
      uid: 191,
      name: 'widgets.svg',
      status: 'done',
      //url: 'https://zos.alipayobjects.com/rmsportal/jkjgkEfvpUPVyRjUImniVslZfWPnJuuZ.png',
      // thumbUrl: 'data:image/png;base64,'//'data:image/png;base64,'
      thumbUrl: 'data:image/png;base64,'
      //thumbUrl: this.domSanitizer.bypassSecurityTrustUrl('data:image/svg+xml;base64,')
      //thumbUrl: './assets/images/widgets.svg')
    }];
    this.resultValue.icon = this.appIconFile.list[0].thumbUrl;
    this.appIconFileMax.list = [{
      uid: -1,
      name: 'widgetsMax.svg',
      status: 'done',
      thumbUrl: 'data:image/png;base64,'
    }];
    this.resultValue.largeIcon = this.appIconFileMax.list[0].thumbUrl;
  }
  /**
   * 随机数令牌
   */
  guid() {
    return (this.S4() + this.S4() + this.S4() + this.S4() + this.S4() + this.S4() + this.S4() + this.S4());
  }
  S4() {
    return (((1 + Math.random()) * 0x10000) | 0).toString(16).substring(1);
  }
  /**
   * name验证
   * @param name 
   */
  getFormControl(name: string) {
    return this.validateForm && this.validateForm.controls && this.validateForm.controls[name];
  }

  initAppFormData() {
    if (this.isInitedForm) {
      return false;
    }
    this.isInitedForm = true;
    if (this.app) {
      for (let o in this.app) {
        this.componentChange(this.app[o], o);
      }
    }
  }

  ngOnChanges(changes: SimpleChanges) {
    this.needSubmit = changes.needSubmit.currentValue || false;
    if (this.needSubmit) {
      this._submitForm();
    } else {
      this.initValidateForm();
      this.initAppFormData();
    }
  }

  onUploadAppIconFile(files: any[]) {
    if (files.length > 0 && files[0].response) {
      this.resultValue.iconId = files[0].response.id;
      this.resultValue.icon = files[0].thumbUrl;
    } else {
      this.errorFlag = false;
    }
  }

  onUploadAppIconFileMax(files: any[]) {
    if (files.length > 0 && files[0].response) {
      this.resultValue.largeTconId = files[0].response.id;
      this.resultValue.largeIcon = files[0].thumbUrl;
    } else {
      this.errorFlagMax = false;
    }
  }
  onRemoveAppIconFile(files: any[]) {
    this.resultValue.icon = null;
  }
  onRemoveAppIconFileMax(files: any[]) {
    this.resultValue.largeIcon = null;
  }
  onErrorAppIconFile(type: any) { //type == true 格式错误
    this.imageErrorFlag = type;
  }
  onErrorAppIconFileMax(type: any) { //type == true 格式错误
    this.imageErrorFlagMax = type;
  }

  /**
   * 获取所有app的信息
   * @return {[type]} [description]
   */
  private getApplist() {
    let params = {
      page: 1,
      rows: 100
    }
    this.appListService.getApps(params).then((data: any) => {
      this.applistInfo = data.list.map((item: any) => {
        return {
          name: item.name,
          code: item.code
        }
      })
    }).catch((err: any) => {
      console.log(err);
    });
  }


  /**
   * 检验code是否重复
   * 这里使用箭头函数是因为内部取不到this
   * @param  {FormControl} control [description]
   * @return {[type]}              [description]
   */
  checkRepeatCode = (control: FormControl): { [key: string]: any } => {
    let codeRepeat: boolean = false
    if (this.applistInfo && this.applistInfo.length && control.value) {
      for (let item of this.applistInfo) {
        if (item.code.trim() === control.value.trim()) {
          codeRepeat = true
          break
        }
      }
    }
    return codeRepeat ? { 'codeRepeat': { value: control.value } } : null
  }
  /**
   * 检验name是否重复
   * 这里使用箭头函数是因为内部取不到this
   * @param  {FormControl} control [description]
   * @return {[type]}              [description]
   */
  checkRepeatNameAs = async (control: FormControl): Promise<any> => {
    return new Promise((resolve: any, reject: any) => {
      let controlV: string = control.value || ''
      controlV && (controlV = controlV.trim())
      let params: any = {
        name: controlV
      }
      if (this.isEdit) {
        params.appId = this.route.snapshot.params['id']
      }
      this.appListService.checkRepeat(params).then((data: any) => {
        resolve(!data.success ? { 'nameRepeat': { value: control.value } } : null)
      }).catch((e) => {
        console.log(e)
      })
    })
  }

  /**
   * 检验name是否重复
   * 这里使用箭头函数是因为内部取不到this
   * @param  {FormControl} control [description]
   * @return {[type]}              [description]
   */
  checkRepeatCodeAs = async (control: FormControl): Promise<any> => {
    return new Promise((resolve: any, reject: any) => {
      let controlV: string = control.value || ''
      controlV && (controlV = controlV.trim())
      let params: any = {
        code: controlV,
      }
      if (this.isEdit) {
        params.appId = this.route.snapshot.params['id']
      }
      this.appListService.checkRepeat(params).then((data: any) => {
        resolve(!data.success ? { 'codeRepeat': { value: control.value } } : null)
      }).catch((e) => {
        console.log(e)
      })
    })
  }


  /**
   * 检验名称和编码是否重复
   * 这里使用箭头函数是因为内部取不到this
   * @param  {FormControl} control [description]
   * @return {[type]}              [description]
   */
  checkCNameRepeat = (control: FormControl): { [key: string]: any } => {
    let cnameRepeat: boolean = false
    if (this.getFormControl('name') && this.getFormControl('code')) {
      let str1 = this.getFormControl('name').value
      let str2 = this.getFormControl('code').value
      str1 = str1 && str1.trim()
      str2 = str2 && str2.trim()
      if (str1 == str2) {
        cnameRepeat = true
      }
    }
    return cnameRepeat ? { 'cnameRepeat': { value: control.value } } : null
  }

}
