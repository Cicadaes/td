...
export class AddRoleFormComponent implements OnInit {
  ...
  constructor(private service: RolesService, private fb: FormBuilder) {
  }
  ...
  checkNameRepeat = (control: FormControl): { [key: string]: any } => {
    ...
    if(this.role){ // 编辑
        return new Promise((resolve: any, reject: any) => {
            this.service.checkRolseName({ name: controlV,id: this.role.id,tenantId: this.tenantId}).then((data: any) => {
