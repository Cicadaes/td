...
class InferCSVSchema {
  def inferSchema(csvData: Array[String],
                  options: Map[String, String]): Option[StructType] = {

    require(csvData.length > 0, "CSV file is empty!")
    val headerLine = csvData.headOption.get

    val csvOptions = new CSVOptions(options)
    val csvReader = new CsvReader(csvOptions)
    val headerRow = csvReader.parseLine(headerLine)

    val header = if (csvOptions.headerFlag) {
      headerRow.zipWithIndex.map { case (value, index) =>
        if (value == null || value.isEmpty || value == csvOptions.nullValue) s"_c$index" else value
      }
    } else {
      headerRow.zipWithIndex.map { case (value, index) => s"_c$index" }
    }

    val csvArrayData = csvData.drop(1).map(csvReader.parseLine(_))
    val schema = if (csvOptions.inferSchemaFlag) {
      infer(csvArrayData, header, csvOptions)
    } else {
  ...
  def infer(
    ...
    val rootTypes: Array[DataType] =
      csvData.aggregate(startType)(inferRowType(options), mergeRowTypes)
...
object InferCSVSchema{
  def main(args: Array[String]): Unit = {
    val csv = new InferCSVSchema
    val options = Map("inferSchema" -> "true", "header" -> "true")
    val tokenRdd: Array[String] = Array("aa,bb,cc,ee","1,1.0,one,true", "2,2.0,two,true","1,1.0,three,false")

    csv.inferSchema(tokenRdd, options).get.printTreeString()
  }
}