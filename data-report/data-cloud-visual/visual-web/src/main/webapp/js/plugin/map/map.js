define(['app','angularAMD','app/directive/EChart'], function (app,angularAMD) {
    'use strict';
    angularAMD.directive('pluginMap',['$compile',
        function($compile) {
    	return {
    		require: '?ngModel',
    		restrict : 'EA',
            ...
            replace: false,
            template: '<div e-chart e-data="options" e-theme="eTheme" loading="loading" class="hg-p-100"></div>',
            link: function(scope,elem,ngModel) {
            	...
            	scope.createReportLineChartData = function(report, datas){
        			...
        			var yList = datas.y,name,yValue,serises=[],legendData=[],dataList=[],maxData=0;
        			...
        			angular.forEach(yList, function(y){
        				...
        				var serise ={
        		            name: name,
        		            type: 'map',
        		            mapType: 'provinceid' === datas.x.id ? 'china' : 'world', 
        		            zoom : 1,
        		            roam: 'false',
        		            itemStyle:{
        		                normal:{label:{show: 'provinceid' === datas.x.id ? true : false}},
...
//        		            data:dataList
        				...
        				if ('countryid' === datas.x.id) {
        					serise.nameMap = nameMap;
						}
					...
					scope.options = EchartsOptions.map(_chart);
