        ...
        for (let key in sourceData) {
            switch (key) {
                ...
                case 'linetype' : //折线图   
                    ...
                    let res: any = [];
                    let resDate: any = [];
                //     if(styleObj['yAxis_max']=="" || styleObj['yAxis_max']==null){ 
                //     newData[`yAxis_max`]= this.DightNumber(this.MaxData(sourceData));
                //    }
                    newData[`legend_data`] = [];
                    for (let i = 0, j = sourceData.length; i < j; i++) {
                        if (!Utils.isRepeat(resDate, sourceData[i].date)) {
                            resDate.push(sourceData[i].date);
                        }
                ...
                //    }
                    newData[`legend_data`] = [];
                    for (let i = 0, j = sourceData.length; i < j; i++) {
                        if (!Utils.isRepeat(arearesDate, sourceData[i].date)) {
                            arearesDate.push(sourceData[i].date);
                        }
                        if (!Utils.isRepeat(areares, sourceData[i].name)) {
                            areares.push(sourceData[i].name);
                        }
                    }
                ...
                //    }
                    newData[`legend_data`] = [];
                    for (let i = 0, j = sourceData.length; i < j; i++) {
                        if (!Utils.isRepeat(barresDate, sourceData[i].date)) {
                            barresDate.push(sourceData[i].date);
                        }
                        if (!Utils.isRepeat(barres, sourceData[i].name)) {
                            barres.push(sourceData[i].name);
                        }
                    }
                    ...
                    for (let index in barres) {
                        ...
                        // newData[`series_${barres.length-1}_barWidth`] = "25%";
                ...
                case 'striptype' : //条形图
                    let stripres: any = [];
                    let stripresDate: any = [];
                    // newData[`xAxis_max`]= this.DightNumber(this.MaxData(sourceData));
                    newData[`legend_data`] = [];
                   
                    for (let i = 0, j = sourceData.length; i < j; i++) {
                        if (!Utils.isRepeat(stripresDate, sourceData[i].date)) {
                            stripresDate.push(sourceData[i].date);
                        }
                        if (!Utils.isRepeat(stripres, sourceData[i].name)) {
                            stripres.push(sourceData[i].name);
                        }
                    }
