...
export class StageContainer extends BaseContainer {
    ...
    private controlKeyClickBind: any;
    private isControlDown: Boolean = false;

    constructor(el: string, scopeID?: string, historyWidth?: number, historyHeight?: number) {
        super();
    ...
    public set preViewModel(bool: Boolean) {
        ...
        if (bool) {
            //触发事件
            this.resizeHandleSave();
            // this.resizeHandle();
            this.enableSelectChild = false;
            this.enableAlignChild = false;
            // this.enableDragCreateChart = false;
            for (let item of this.allChildren) {
                item.enableEdit = false;
            }
        } else if(bool == null) {
            this.resizeHandleSave();
            // this.resizeHandleNull();
            this.enableSelectChild = true;
            this.enableAlignChild = true;
            // this.enableDragCreateChart = true;
            for (let item of this.allChildren) {
    ...
    private componentSelect(e: any, data: any): void {
        if (this.isControlDown) {
            ...
            for (var item of this.allChildren) {
                if (item.scopeID == data.scopeID) {
                    item.isSelect = true;
                } else {
                    item.isSelect = false;
    ...
    private dragCreateHandle(e: any): void {
        ...
        if (chartType) {
            ...
            if (component) {
                let componentContainer: ComponentContainer = new ComponentContainer(component,component.scopeID);
    ...
    public set enableSelectChild(select: Boolean) {
        // this.select(select);
    }

    public set enableAlignChild(align: Boolean) {
        if (align) {
            EventEmitter.register("relativeStage", (e: any, data: any) => {
                this.isRelativeStage = data;
            }, this);
            EventEmitter.register(EventType.ALIGN.action, this.actionAlignHandle, this);
        } else {
            EventEmitter.unRegister(EventType.ALIGN.action, this.actionAlignHandle, this);
        }
    }
    ...
    private resizeHandleSave(): void {
        if (this.timer) this.timer.stopTimeOut();
        let scaleValue: any = 1;
        this.publicResize(scaleValue)
    }

    //重置舞台
    private resizeHandle(): void {
        if (this.timer) this.timer.stopTimeOut();
        let saveStageSize = document.documentElement.clientWidth;
        if(this.widthSize == undefined){
            this.widthSize = this.width;
        }
        let scaleValue: any = (saveStageSize / this.widthSize).toFixed(2);
        this.publicResize(scaleValue)
    }

    private resizeHandleReturn(): void {
        if (this.timer) this.timer.stopTimeOut();
        let saveStageSize = document.documentElement.clientWidth;
        let scaleValue: any = (this.historyWidth / saveStageSize).toFixed(2);
        this.publicResize(scaleValue);
    }

    private resizeHandleNull(): void {
        if (this.timer) this.timer.stopTimeOut();
        this.publicResize(1);
    }


    private publicResize(scaleValue:any){
        for (let child of this.allChildren) {
            child.width = child.width * scaleValue;
            child.height = child.height * scaleValue;
            child.x = child.x * scaleValue;
            child.y = child.y * scaleValue;
        }
        this.historyWidth = this.width;
        this.historyHeight = this.height;
    }
    ...
    public stagechange(e:any,data:any): void{
        this.sizeFormat = data.result.size;
        this.addStyle('margin','20px auto');
        this.addStyle('width',data.result['width'] + 'px');
        this.addStyle('height',data.result['height'] + 'px');
        console.log(data.result)
        console.log(this.sizeFormat)
    }
