...
RenderDom = {
    ...
    monthList: function(con, options) {
        ...
        for (var i = 1; i < 13; i++) {
            ...
            html += '<li><span target-type="TD_MonthPicke_month" data-type="' + date + '" class="TD_MonthPicke_month ' + this.statusClass(con, date) + '">' + m + '月</span></li>';
        };
...
Operation = {
    init: function(myThis) {
        ...
        myThis.selectedMonth = [$(".TD_MonthPicke_Frame_Content span.chooes:first").data('type'), $(".TD_MonthPicke_Frame_Content span.chooes:last").data('type')];
        //debugger
    },
            ...
            switch (type) {
                ...
                case "TD_MonthPicke_month":
                    ...
                    if (hashClass.indexOf("disable") <= 0) {
                        ...
                        $('[target-type= "TD_MonthPicke_month"].chooes').removeClass("special-choice");
                        $('[target-type= "TD_MonthPicke_month"].chooes').removeClass("chooes");
                        $target.addClass("special-choice");
                        example.o.date = $target.attr("data-type");
                        var msg = example.o.date;

                        if(example.o.multiChooseType == 'single'){
                            $target.parents('.TD_MonthPicke_Frame_Content').find('.TD_MonthPicke_MonthList span').removeClass('special-choice');
                            $target.addClass("special-choice");
    ...
    selectMonth: function(example) {
        ...
        preSpan.each(function(index) {
            if ($(preSpan[index]).attr("data-type") == example.selectedMonth[sIndex]) {
                start = index;
                if(example.o.multiChooseType == 'multi'){
