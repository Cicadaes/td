  ...
  private decideFilterType(result: any) {
    ...
    switch (result.selectName) {
      ...
      case "select_3":
        ...
        let title = this.decideMouth(date.getFullYear() + "-" + prefix + (date.getMonth() + 1));
        ...
        for (let i = 0, j = date.getMonth(), k = date.getFullYear(); i < 13; i++, j--) {
          if (j < 0) {
            j = 11;
            k = k - 1;
          }
        ...
        this.renderSelectListHtml(listDom, list.splice(1));
        break;
    }
  }
  ...
  private postChange(postQuery: any, type: any) {
    switch (type) {
      case "brand":
        postQuery = {
          "datasource_id": 55,
          "dimensions": [{"field": "brand_name", "alias": "key"}, {
            "field": "brand_value",
            "alias": "value"
          }]
        };
