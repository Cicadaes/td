    ...
    public renderListByArr(container:any,arr:any,str:any){
        ...
        for(let i =0;i < arr.length; i++){
            ...
            for(let j=0; j < arr[i].list.length; j++){
                html += "<li class='fl'><input type='radio' id='" + arr[i].list[j].code + str + "' name='"+radioName+"' value='" + arr[i].list[j].code +"' /><label for='" + arr[i].list[j].code + str + "'>" + arr[i].list[j].val+"</label></li>"
            }
            html += "</div>"
        }
