...
//    this.clearFormData();
    ...
    initSelectEvent(value: any, field: any) {
        ...
        let apiUrl = this.service.queryEventPagingUrl + parentId + '&page=';
        if (initValue) {
            apiUrl = this.service.queryEventPagingUrl + parentId + '&id=' + initValue + '&page=';
        }
    ...
    save() {
        ...
        newData.dateRange = this.globals.dateFormatNumber(this.validateForm.value.dateRange[0], '') +
            '~' +
            this.globals.dateFormatNumber(this.validateForm.value.dateRange[1], '');
        if (!this.switchValue) {
