...
export class FunnelSeeComponent extends BaseComponent implements OnInit, OnChanges {
    ...
    constructor(private funnelSeeService: FunnelSeeService,
    ...
    ngOnInit() {
        ...
        $scope.chartOption = {
            color: ['#34508c', '#2b85e4', '#1cb6fb', '#1bdbf5', '#80f2da', '#c1f9d6', '#fcf4ae', '#fbe790', '#ffbd6e', '#fb7b49'],
            title: {
                text: '',
                left: 'center',
                top: '20px',
                ...
                },
                {
                    label: {
                        ...
                        formatter: function (val) {
                            ...
                            newName = newName.replace(/[^\x00-\xff]/g, '$&\x01').replace(/.{20}\x01?/g, '$&\n').replace(/\x01/g, '');
                            return newName;
                        }
                    },
                    labelLine: {
    ...
    initChartOption($next ?: any) {
        ...
        let aMin = Math.min.apply(Math, a.map(function (o) {
            return o.value;
        }));
        const aMax = Math.max.apply(Math, a.map(function (o) {
            return o.value;
        }));
        if (aMin !== aMax) {
            aMin = 0;
        ...
        let bMin = Math.min.apply(Math, b.map(function (o) {
            return o.value;
        }));
        const bMax = Math.max.apply(Math, b.map(function (o) {
            return o.value;
        }));
        if (bMin !== bMax) {
            bMin = 0;
        ...
        let eMin = Math.min.apply(Math, e.map(function (o) {
            return o.value;
        }));
        const eMax = Math.max.apply(Math, e.map(function (o) {
            return o.value;
        }));
        if (eMin !== eMax) {
            eMin = 0;
    ...
    initSelectEventMultiple() {
        if ($scope.vm.filter.propValue) {
            ...
            this._selectEventMultiple = {
                apiUrl: this.funnelSeeService.queryEventUrl + value + '&rows=20&page=',
                apiType: 'get',
                apiData: true,
                apiSearch: true,
    ...
    onSelectEventMultiple(value: any) {
        ...
        console.dir([idsValue, $scope.vm.filter.propValueValue]);
    }
