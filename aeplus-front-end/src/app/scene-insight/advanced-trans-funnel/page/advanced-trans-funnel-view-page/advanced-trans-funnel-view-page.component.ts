...
export class AdvancedTransFunnelViewPageComponent extends BaseComponent implements OnInit {
    ...
    constructor(private fb: FormBuilder,
                public service: AdvancedTransFunnelService,
            ...
            tooltip: {
                ...
                formatter: function (params) {
                    ...
                    for (let i = 0, l = params.length; i < l; i++) {
                        if (i === 0) {
                            relVal += '<br/>' + '<span style="line-height: 10px;\n' +
                                ...
                                '    margin-bottom: 1px;\n' +
                                '    background: #2D8CF0;">' +
                                '</span>' + params[i].seriesName + ' : ' + that.globals.toThousands(params[i].value);
                        } else {
                            relVal += '<br/>' + '<span style="line-height: 10px;\n' +
                                '    display: inline-block;\n' +
                                '    height: 6px;\n' +
...
//          interval: 5,
                       ...
                       for(let i = 0; i < searchData.queryFilters.length; i++){
                           that.moreSearchList[i] = {
                            ...
                            if(searchData.queryFilters[i].displayType === 'Tag') {
                                that.getRightList(i, that.moreSearchList[i],searchData.queryFilters[i].values);
                                ...
                                if(searchData.queryFilters[i].operator === 'range'){
                                    that.moreSearchList[i].values = [new Date(searchData.queryFilters[i].values[0]),new Date(searchData.queryFilters[i].values[1])];
                                }else {
                                    that.moreSearchList[i].value = new Date(searchData.queryFilters[i].values[0]);
                                }
                            }else if(searchData.queryFilters[i].displayType === 'Integer' || searchData.queryFilters[i].displayType === 'Double'){
                                if(searchData.queryFilters[i].operator === 'range'){
    ...
    getRightList(index: number, item: any,values: any[]) {
        ...
        that.service.getEventList(tempJson).subscribe((data: any) => {
...
//    json['eventTypeId'] = that.selecteEventType;
        ...
        that.service.search(json).subscribe((data: any) => {
            console.dir([data]);
...
//                            x: xPos + '%',
    ...
    queryChildTableData(params: any) {
        ...
        this.service.search(queryParams).subscribe((data: any) => {
    ...
    downloadData() {
        ...
        const data = {
            ...
            title: `场景洞察-高级转化漏斗-明细数据-数据导出-${dateRangeStr}`,
    ...
    goUserPage(param: any, group: any) {
        ...
        this.goToPage('用户列表', '/scene-insight/advanced-trans-funnel/users', {queryParams: JSON.stringify(queryParams)});
    }
