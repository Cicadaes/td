...
import {NzModalRef} from '../../../../node_modules/ng-cosmos-ui';
    ...
    ngOnInit() {
        ...
        that.paper.on('blank:pointerclick', async() => {
            if (that.pcs.nodeData && that.pcs.nodeData.groupName && that.pcs.isPipelineEdit) {
                await new Promise((resolve, reject) => {
                    that.pipelineService.saveSegmentGroup(that.pcs.campaignId, that.pcs.nodeData.groupName, localStorage.getItem('productId')).subscribe((data: any) => {
                        if (data.code === 200) {
        ...
        that.paper.on('blank:pointerdown', (e: any, x: any, y: any) => {
            ...
            svg.node.onmousemove = (e: any) => {
                that.orignX = that.utiles.floatMul(that.utiles.floatSub(that.utiles.floatDiv(e.offsetX, that.scaleX), x), that.scaleX);
                that.orignY = that.utiles.floatMul(that.utiles.floatSub(that.utiles.floatDiv(e.offsetY, that.scaleY), y), that.scaleY);
...
//            }
        ...
        that.graph.on('change:source change:target', (link: any) => {
            ...
            that.linkSetTimeoutId[link.id] = setTimeout(() => {
                ...
                if (link.data && link.data.id && sourceNode && sourceNode.data && (sourceNode.data.type === 'split' || sourceNode.data.type === 'trigger')) {
                ...
                if (sourceId && targetId) {
                    ...
                    if (link.data && link.data.name && sourceNode && sourceNode.data && (sourceNode.data.type == 'split' || sourceNode.data.type == 'trigger')) {
                        return;
                    }
                    that.removeFlag = true;
                    that.graph.removeCells([link]);
        ...
        that.graph.on('change:vertices', (link: any) => {
            ...
            that.moveLinkSetTimeoutId = setTimeout(() => {
                if (link.data) {
                    for (let i = 0; i < that.diagramObj.edgeDefinitionList.length; i++) {
                        if (that.diagramObj.edgeDefinitionList[i].id === link.data.id) {
                            that.diagramObj.edgeDefinitionList[i]['vertices'] = _.cloneDeep(link.attributes.vertices);
                            break;
                        }
                    }
                    that.updatePipeLine();
                ...
                if (response.code === 200) {
                    ...
                    if (that.diagramObj.nodeDefinitionList.length === 0) {
                        that.diagramObj.nodeDefinitionList.push({
                            ...
                            serial: that.guid(),
                            "pipelineDefinitionId": that.pipelineId,
                            "icon": "/aeplus/assets/images/pipeline/component-entrance.svg",
                            "name": "入口",
                            "type": "entrance",
                            "x": 122,
                            "y": 120,
            ...
            if (!data.deleteType) {
                for (let i = 0; i < linksLength; i++) {
                    ...
                    if (linkIndex == data.index) {
                        ...
                        that.removeFlag = false;
                    } else if (linkIndex != 10 && linkIndex > data.index) {
    ...
    drop(e: any) {
        ...
        dataInfo.x = that.utiles.floatDiv(parseInt(e.offsetX) - that.utiles.floatMul(that.orignX, that.scaleX), that.scaleX);
        dataInfo.y = that.utiles.floatDiv(parseInt(e.offsetY) - that.utiles.floatMul(that.orignY, that.scaleY), that.scaleY);
...
//                },
    ...
    initOperatorModel(type?: string) {
        ...
        const model = joint.shapes.basic.Rect.extend({
            markup: '<g class="rotatable"><text class="label top"/><image /><text class="label"/><rect class="rect"></rect></g>',
