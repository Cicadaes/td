...
export class TriggerPanelComponent implements OnInit {
    ...
    constructor(public pipelineService: PipelineService,
        ...
        // 获取事件列表
        that.pipelineService.getEventListForTimer(that.pcs.isPipelineEdit, {triggerFlag: 1}).subscribe((result: any) => {
...
//                    that.eventsAll.push(data);
                ...
                // 获取指标列表 同时满足指标列表
                that.pipelineService.getTriggerTargetList(2, that.pcs.isPipelineEdit).subscribe((result: any) => {
                    if (result && result.code === 200) {
