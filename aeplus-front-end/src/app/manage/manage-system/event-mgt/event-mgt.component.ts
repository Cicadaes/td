import {Component, ViewChild, Injector, OnInit, OnChanges} from '@angular/core';
...
export class EventMgtComponent extends BaseComponent implements OnInit {
    ...
    @ViewChild(EventMgtDiscontinuationComponent) private discontinuation: EventMgtDiscontinuationComponent;
...
//            $scope.initContentData();
    ...
    initSearchEventType($next ?: any) {
        ...
        $scope.eventMgtService.post_getEventTypeList().subscribe((response: any) => {
            ...
            if (a && a.length > 0) {
                ...
                $scope.handlerSearch_appVersion();
    ...
    handlerSearch_appVersion() {

        $scope.vm.search.appVersionLoading = true;
        $scope.eventMgtService.post_getAppVersionList($scope.vm.search.eventTypeValue).subscribe((response: any) => {
            let a, b, i;
            for (a = [], i = 0; b = response.list[i]; i++) {
                a.push({
                    label: b.version,
            ...
            $scope.vm.search.appVersionLoading = false;
    ...
    handlerEventExport() {
        ...
        $scope.vm.content.dataByEventExport =
            '/reportservice/ExportEventExcelSerlet' +
            '?productid=' + $scope.productId +
            '&eventname=' + ($scope.vm.search.eventNameValue || '') +
            '&eventtype=' + ($scope.vm.search.eventTypeValue || '') +
            '&version=' + ($scope.vm.search.appVersionValue || '')
        ;
