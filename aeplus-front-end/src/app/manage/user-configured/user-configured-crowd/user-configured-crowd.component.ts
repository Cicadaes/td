...
export class UserConfiguredCrowdComponent extends BaseComponent implements OnInit, OnChanges {
    ...
    // 查询属性个数  人群显著标签配置
    getAttributeCount() {
        this.userConfiguredService.getAttributeCount({key: 'CROW_SIGNIFICANCE'}).subscribe(
            (response: any) => {
    ...
    deleteFunc() {
        this.portrayalTableLoading = true;
        this.userConfiguredService.daletePortrait(this._item.id, this._item.reportId, this._item.productId).subscribe((response: any) => {
    ...
    configReport(item: any) {
        ...
        const aepUrl = window.location.origin + `/aeplus/#/manage/user-configured/crowd`;
        ...
        const obj = {
            dmp: {
                url: aepUrl,
                header: [
                    {name: '用户配置', url: aepUrl0},
                    {name: '用户分群配置', url: aepUrl0},
                    {name: '人群画像配置', url: aepUrl1},
                    {name: item.name, url: ''}
                ],
        ...
        const src = this.reportUrl + `/report-uop/#/report-detail-uop/${item.reportId}`;
