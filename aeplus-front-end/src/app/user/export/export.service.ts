...
export class ExportService extends CurdService {
    ...
    // 获取信息属性组查询接口
    getListDetails() {
        const url = `${this.baseUrl}/meta/metaAttribute/listDetails/${this.productId}`;
        return this.http.get(url).pipe(catchError(this.handleError));
    }

    // 用户配置通用配置查询接口
    getProductConfig(source: number, type: number) {
//        const url = `${this.baseUrl}/user/queryProductConfig/${this.productId}/${source}/${type}`;
    }

    exportCrowd(crowdId: number, attribute: any) {
        const url = `${this.baseUrl}/crowd/crowd/export/${crowdId}?crowdId=${crowdId}`;
        return this.http.put(url, attribute).pipe(catchError(this.handleError)).pipe(catchError(this.handleError));
    }
