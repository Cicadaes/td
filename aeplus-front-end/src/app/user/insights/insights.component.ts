...
export class InsightsComponent extends BaseComponent implements OnInit, AfterViewInit {
                ...
                that.insightsService.getParam(keys).subscribe((result: any) => {
                    if (result.code === 200) {
                        ...
                        for (let i = 0; i < that.otherIds.length; i++) {
                            that.otherIds[i]['percent'] = +that.otherIds[i]['value'] === 0 ? 0 : ((+that.otherIds[i]['value'] / count) * 100).toFixed(2);
            ...
            if (data.code === 200) {
                ...
                if (that.portraits.length > 0) {
                    that.iframeUrl = that.domSanitizer.bypassSecurityTrustResourceUrl(
                        that.reportUrl + '/report-uop/#/publish/' + that.portraits[0].reportId + '?crowd_id=' +
                        that.crowdId + '&product_id=' + that.productId + '&token=' + localStorage.getItem('token')
                    );
                }
            } else {
                that.notification.create('warning', '错误提示', data.message);
    ...
    showChars(index: any) {
        ...
        that.iframeUrl = that.domSanitizer.bypassSecurityTrustResourceUrl(
            that.reportUrl + '/report-uop/#/publish/' + that.portraits[index].reportId + '?crowd_id=' +
            that.crowdId + '&product_id=' + that.productId + '&token=' + localStorage.getItem('token')
        );
        setTimeout(() => {
            that.showIframe = true;
