...
export class ControlPanelComponent implements OnInit, OnChanges {
    ...
    requestPeriod2(period: string, type: boolean) {
        ...
        if (period === 'Year') {
            ...
            // 当前周期为1，回退上一年最后一个周期
            if (dateParamOld.valueOfYear === 1) {
                const lastWeek = this.datePickerService.weekListMap[dateParamOld.year - 1] ? this.datePickerService.weekListMap[dateParamOld.year - 1].length : 52;
