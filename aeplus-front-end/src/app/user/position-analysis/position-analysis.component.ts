...
export class PositionAnalysisComponent extends BaseComponent implements OnInit, OnChanges {
    ...
    constructor(private positionService: PositionAnalysisService,
    ...
    toThousands(numIn: any) {
        let num = (numIn || 0).toString(), result = '';
        while (num.length > 3) {
    ...
    onDataChange(event) {
        ...
        if (this.zoomName === 'world') {
            ...
            if (event.zoomName) {
                // 恢复中心点和缩放
                const center = this.chinaCenter;
                const BMap = window['BMap'];
                const point = new BMap.Point(center[0], center[1]);

                const _that = this;
                setTimeout(() => {
                    _that.chinaMapInstance.getMap().setCenter(point);
                    _that.chinaMapInstance.getMap().setZoom(5);
                }, 100);
            }
        }
    }
    ...
    selectHit(name: string) {
        ...
        if (this.zoomName === 'world') {
            ...
            const center = GeoData.chinaCenterMap[name] || this.chinaCenter;
            const BMap = window['BMap'];
            const point = new BMap.Point(center[0], center[1]);
            this.chinaMapInstance.getMap().setCenter(point);
            this.chinaMapInstance.getMap().setZoom(name ? (name === '香港' || name === '澳门' ? 10 : 7) : 5);
            let renderData = this.chinaData;
            if (this.chinaOverlay.getData() > 0) {
                renderData = this.chinaOverlay.getData();
            }
            const selectList = [];
            ...
            this.chinaOverlay.setSelectedList(selectList);
            this.chinaOverlay.refresh();
        }
        ...
        this.worldChartOption = {
            tooltip: {
                ...
                formatter: function (params) {
                    ...
                    + (params.data ? params.data.objectName : params.name) + '<br/>' +
                        '数量 ' + (params.value ? _that.toThousands(params.value) : 0) + '<br/>' +
                        '占比 ' + (params.data ? params.data.rate : '0.0') + ' %' + '<br/>' +
                        _that.subTitle;
                },
                textStyle: {
                    fontSize: 12
    ...
    // 初始化中国地图
    initChinaMapSimple() {
        const InMap = window['inMap'];
        this.chinaMapInstance = new InMap.Map({
            id: 'china-map',
            skin: 'Blue',
        ...
        this.chinaMapInstance.map.disableScrollWheelZoom();

        const _those = this;
        this.chinaOverlayConfig = {
            tooltip: {
                show: true,
                formatter: function (params) {
                    return 'NO.' + params.index + ' ' + params.name + '<br/>' +
                        '数量 ' + (params.count ? _those.toThousands(params.count) : 0) + '<br/>' +
                        '占比 ' + (params.rate) + ' %' + '<br/>' +
                        _those.subTitle;
                },
                offsets: {
        ...
        const overlay = new InMap.BoundaryOverlay(this.chinaOverlayConfig);

        this.chinaOverlay = overlay;
        this.chinaMapInstance.add(overlay);
    }

    // 查询世界地图数据
    requestWorldData(dateParam: any) {
        // 请求世界地图数据，返回后加载世界地图
        this.positionService.countryList(dateParam).subscribe((response: any) => {
            let maxValue = 0;
            const dataTmp = [];
    ...
    changeChinaMap(dataMap: any, chinaRateMap: any, maxValue: any, indexMap: any) {
        ...
        if (maxValue === 0) {
            ...
            for (const key in dataMap) {
                if (dataMap.hasOwnProperty(key)) {
                    ...
                    splitList.push({
                        start: dataMap[key],
                        end: this.chinaTop10List[indexMap[key]] ? (indexMap[key] - 2 > 0 ?
                            this.chinaTop10List[indexMap[key] - 2].value : dataMap[key] + 1) : dataMap[key] + 1,
                        borderColor: '#FFFFFF',
                        borderWidth: 1,
                        backgroundColor: bg,
                    });
        ...
        setTimeout(() => {
            this.chinaOverlay.setPoints(this.chinaData);
            this.chinaOverlayConfig.style.splitList = splitList;
            this.chinaOverlay.setOptionStyle(this.chinaOverlayConfig);
        }, 100);
    }
